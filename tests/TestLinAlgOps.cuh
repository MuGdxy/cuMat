...
void testLinAlgOpsReal()
{
    ...
    for(int i = 0; i < batches; ++i)
    {
        ...
        auto slice =
            Matrix<Scalar, Dims, Dims, 1, Flags>::fromEigen(inputMatricesHost[i]);
    ...
    //1. Determinant
    {
        ...
        }
        {
            ...
            auto determinantDevice =
                ((inputMatrixDevice + 0.0f).determinant() + 0.0f).eval();
            REQUIRE(determinantDevice.rows() == 1);
            REQUIRE(determinantDevice.cols() == 1);
            REQUIRE(determinantDevice.batches() == batches);
            std::vector<Scalar> determinantHost(batches);
    ...
    TestInverseWithDetHelper<Scalar, Flags, Dims, (Dims <= 4)>::run(inputMatricesHost,
                                                                    inputMatrixDevice);
}
template <int Dims>
void testlinAlgOps2()
