...
void testMatrixToEigen(const _Matrix& m)
{
    ...
    cuMat::KernelLaunchConfig cfg =
        ctx.createLaunchConfig3D(sx, sy, 1, TestEigenWriteCoeffKernel<_Matrix>);
...
TEST_CASE("matrix_to_eigen", "[eigen-interop]")
{
    ...
    testMatrixToEigen(
        cuMat::Matrix<float, cuMat::Dynamic, cuMat::Dynamic, 1, cuMat::ColumnMajor>(32, 6, 1));
    ...
    testMatrixToEigen(
        cuMat::Matrix<float, cuMat::Dynamic, cuMat::Dynamic, 1, cuMat::RowMajor>(32, 6, 1));
}
...
void testMatrixFromEigen(const _Matrix& m)
{
    ...
    cuMat::KernelLaunchConfig cfg =
        ctx.createLaunchConfig3D(sx, sy, 1, TestEigenReadCoeffKernel<matrix_t>);
