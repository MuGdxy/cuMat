...
#define UNARY_OP_HELPER(cuMatFn, eigenFn, min, max)                                                                                    \
    ...
    {                                                                                                                                  \
        std::vector<Eigen::Matrix<_Scalar, Eigen::Dynamic, Eigen::Dynamic, cuMat::eigen::StorageCuMatToEigen<_Flags>::value>> m_host(  \
            batches);                                                                                                                  \
        for(cuMat::Index i = 0; i < batches; ++i)                                                                                      \
        {                                                                                                                              \
            m_host[i].setRandom(rows, cols);                                                                                           \
            m_host[i] = _Scalar((min) + ((max) - (min)) / 2)                                                                           \
                        + _Scalar(((max) - (min)) / 2) * m_host[i].array();                                                            \
        }                                                                                                                              \
                                                                                                                                       \
        cuMat::Matrix<_Scalar, _Rows, _Cols, _Batches, _Flags> m_device(rows, cols, batches);                                          \
        for(cuMat::Index i = 0; i < batches; ++i)                                                                                      \
        {                                                                                                                              \
            auto slice = cuMat::Matrix<_Scalar, _Rows, _Cols, 1, cuMat::ColumnMajor>::fromEigen(                                       \
                m_host[i]);                                                                                                            \
            m_device.block(0, 0, i, rows, cols, 1) = slice;                                                                            \
        }                                                                                                                              \
                                                                                                                                       \
        cuMat::Matrix<_Scalar, _Rows, _Cols, _Batches, _Flags> m_device1 =                                                             \
            m_device.cuMatFn();                                                                                                        \
        cuMat::Matrix<_Scalar, _Rows, _Cols, _Batches, _Flags> m_device2 =                                                             \
            CUMAT_FUNCTION_NAMESPACE eigenFn(m_device);                                                                                \
                                                                                                                                       \
        std::vector<Eigen::Matrix<_Scalar, Eigen::Dynamic, Eigen::Dynamic, cuMat::eigen::StorageCuMatToEigen<_Flags>::value>> m_host1( \
            batches);                                                                                                                  \
        std::vector<Eigen::Matrix<_Scalar, Eigen::Dynamic, Eigen::Dynamic, cuMat::eigen::StorageCuMatToEigen<_Flags>::value>> m_host2( \
            batches);                                                                                                                  \
        for(cuMat::Index i = 0; i < batches; ++i)                                                                                      \
        {                                                                                                                              \
