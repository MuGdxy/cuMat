...
#define TEST_SIZE_F1(type, flags, rowCompile, rowRuntime, colCompile, colRuntime, batchCompile, batchRuntime) \
    do                                                                                                        \
    {                                                                                                         \
        cuMat::Matrix<type, rowCompile, colCompile, batchCompile, flags> m(                                   \
            rowRuntime, colRuntime, batchRuntime);                                                            \
...
TEST_CASE("instantiation_throws", "[matrix]")
{
    ...
    REQUIRE_THROWS(
        (cuMat::Matrix<float, cuMat::Dynamic, cuMat::Dynamic, cuMat::Dynamic, 0>(-1, 6, 4)));
    REQUIRE_THROWS(
        (cuMat::Matrix<float, cuMat::Dynamic, cuMat::Dynamic, cuMat::Dynamic, 0>(8, -1, 4)));
    REQUIRE_THROWS(
        (cuMat::Matrix<float, cuMat::Dynamic, cuMat::Dynamic, cuMat::Dynamic, 0>(8, 6, -1)));
}
...
TEST_CASE("write_raw", "[matrix]")
{
    ...
    cuMat::KernelLaunchConfig cfg =
        ctx.createLaunchConfig1D((unsigned int)m.size(), TestMatrixWriteRawKernel<Mat_t>);
...
TEST_CASE("read_raw", "[matrix]")
{
    ...
    cuMat::KernelLaunchConfig cfg =
        ctx.createLaunchConfig1D((unsigned int)m.size(), TestMatrixReadRawKernel<Mat_t>);
...
TEST_CASE("write_coeff_columnMajor", "[matrix]")
{
    ...
    cuMat::KernelLaunchConfig cfg =
        ctx.createLaunchConfig3D(sx, sy, sz, TestMatrixWriteCoeffKernel<Mat_t>);
...
void testDeepClone()
{
    typedef typename cuMat::Matrix<T, cuMat::Dynamic, cuMat::Dynamic, cuMat::Dynamic, cuMat::RowMajor> MatR;
    typedef typename cuMat::Matrix<T, cuMat::Dynamic, cuMat::Dynamic, cuMat::Dynamic, cuMat::ColumnMajor> MatC;
    T    data[2][2][3]{{{1, 2, -1}, {3, 4, -2}}, {{5, 6, -3}, {7, 8, -4}}};
    MatR matR = MatR::fromArray(data);
    MatC matC = matR + 0;
