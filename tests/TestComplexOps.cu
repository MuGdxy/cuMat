...
void testUnary()
{
    ...
    SECTION("inverse")
    {
        type expected[1][3][3]{{
            ...
            {type(0.41237113402f, -0.927835051546f),
             type(-0.641025641f, -0.1282051282f),
             type(0.17421602787f, 0.1393728223f)}  //mixed
        }};
...
void testBinary()
{
    ...
    type dataLeft[1][1][3]{{{type(0.4f, 0.9f), type(-1.5f, 0.3f), type(3.5f, -2.8f)}}};
    CMatrix left = CMatrix::fromArray(dataLeft);

    type dataRight[1][1][3]{{{type(-3.5f, -0.4f), type(0.1f, 2.6f), type(-1.3f, -1.1f)}}};
    CMatrix right = CMatrix::fromArray(dataRight);

    SECTION("add")
    {
        type expected[1][1][3]{{{type(-3.1f, 0.5f), type(-1.4f, 2.9f), type(2.2f, -3.9f)}}};
        assertMatrixEquality(CMatrix::fromArray(expected), left + right, 1e-5);
    }
    SECTION("sub")
    {
        type expected[1][1][3]{{{type(3.9f, 1.3f), type(-1.6f, -2.3f), type(4.8f, -1.7f)}}};
        assertMatrixEquality(CMatrix::fromArray(expected), left - right, 1e-5);
    }
    SECTION("mul")
    {
    ...
    SECTION("div")
    {
        type expected[1][1][3]{{{type(-0.141821f, -0.240935f),
                                 type(0.0930576f, 0.580502f),
                                 type(-0.506897f, 2.58276f)}}};
        assertMatrixEquality(CMatrix::fromArray(expected), left.cwiseDiv(right), 1e-5);
    }
    SECTION("pow")
    {
        type expected[1][1][3]{{{type(-1.05748f, 1.29578f),
                                 type(0.0000841987f, 0.000487057f),
                                 type(0.0484779f, -0.0472866f)}}};
...
void testRealImag()
{
    ...
    {
        Real expectedReal[1][3][3]{{{1, 6, -4}, {0, 0, 0}, {0.4f, -1.5f, 3.5f}}};
        Real expectedImag[1][3][3]{{{0, 0, 0}, {5, -3, 0.3f}, {0.9f, 0.3f, -2.8f}}};
    ...
    {
        Real realPart[1][3][3]{{{1, 6, -4}, {0, 0, 0}, {0.4f, -1.5f, 3.5f}}};
        Real imagPart[1][3][3]{{{0, 0, 0}, {5, -3, 0.3f}, {0.9f, 0.3f, -2.8f}}};
        CMatrix newMat(3, 3);
        newMat.setZero();
