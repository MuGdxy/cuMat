...
CUMAT_STRONG_INLINE __host__ __device__ real3 make_real3(real x, real y, real z)
{
...
struct real3x3
{
    ...
    explicit __host__ __device__ CUMAT_STRONG_INLINE real3x3(real v)
        : r1(make_real3(v, v, v))
        , r2(make_real3(v, v, v))
        , r3(make_real3(v, v, v))
    {
    }
    __host__ __device__ CUMAT_STRONG_INLINE real3x3(const real3& r1,
                                                    const real3& r2,
                                                    const real3& r3)
        : r1(r1)
        , r2(r2)
    ...
    {
        return (r1.x * r2.y * r3.z + r1.y * r2.z * r3.x + r1.z * r2.x * r3.y
                - r1.z * r2.y * r3.x - r1.x * r2.z * r3.y - r1.y * r2.x * r3.z);
    }
    ...
    __host__ __device__ CUMAT_STRONG_INLINE real3x3 inverse() const
    {
        real detInv =
            real(1)
            / (r1.x * r2.y * r3.z + r1.y * r2.z * r3.x + r1.z * r2.x * r3.y
               - r1.z * r2.y * r3.x - r1.x * r2.z * r3.y - r1.y * r2.x * r3.z);
        return real3x3(detInv
                           * make_real3(r2.y * r3.z - r2.z * r3.y,
                                        r1.z * r3.y - r1.y * r3.z,
                                        r1.y * r2.z - r1.z * r2.y),
                       detInv
                           * make_real3(r2.z * r3.x - r2.x * r3.z,
                                        r1.x * r3.z - r1.z * r3.x,
                                        r1.z * r2.x - r1.x * r2.z),
                       detInv
                           * make_real3(r2.x * r3.y - r2.y * r3.x,
                                        r1.y * r3.x - r1.x * r3.y,
                                        r1.x * r2.y - r1.y * r2.x));
    }
    ...
    {
        return real3x3(make_real3(r1.x, r2.x, r3.x),
...
struct NumTraits<real3>
{
    ...
    static constexpr CUMAT_STRONG_INLINE real epsilon()
    {
...
template <>
struct ProductElementFunctor<real3x3, real3, ProductArgOp::NONE, ProductArgOp::NONE, ProductArgOp::NONE>
{
    using Scalar = real3;
    static CUMAT_STRONG_INLINE __device__ Scalar mult(const real3x3& left, const real3& right)
    {
        return make_real3(dot(left.r1, right), dot(left.r2, right), dot(left.r3, right));
    }
};
...
struct AsDiagonalFunctor<real3>
{
    ...
    {
        return real3x3(make_real3(v.x, 0, 0), make_real3(0, v.y, 0), make_real3(0, 0, v.z));
    }
};
...
struct UnaryMathFunctor_cwiseInverseCheck<real3>
{
  public:
    ...
    {
        return make_real3((v.x == real(0) ? real(1) : real(1) / v.x),
...
struct BinaryMathFunctor_cwiseDot<real3>
{
  public:
    typedef real ReturnType;
    __device__ CUMAT_STRONG_INLINE real
    operator()(const real3& x, const real3& y, Index row, Index col, Index batch) const
    {
        return dot(x, y);
    }
};
...
TEST_CASE("Blocked CG", "[CG]")
{
    ...
    pattern.nnz  = 7;
    pattern.JA =
        SPattern::IndexVector::fromEigen((Eigen::VectorXi(4) << 0, 2, 5, 7).finished());
    pattern.IA = SPattern::IndexVector::fromEigen(
        (Eigen::VectorXi(7) << 0, 1, 0, 1, 2, 1, 2).finished());
    SMatrix3x3           mat(pattern);
    std::vector<real3x3> data1 = {
        real3x3(make_real3(2, -1, 0), make_real3(-1, 2, -1), make_real3(0, -1, 2)),
        real3x3(make_real3(0, 0, 0), make_real3(0, 0, 0), make_real3(-1, 0, 0)),
        real3x3(make_real3(0, 0, -1), make_real3(0, 0, 0), make_real3(0, 0, 0)),
        real3x3(make_real3(2, -1, 0), make_real3(-1, 2, -1), make_real3(0, -1, 2)),
        real3x3(make_real3(0, 0, 0), make_real3(0, 0, 0), make_real3(-1, 0, 0)),
        real3x3(make_real3(0, 0, -1), make_real3(0, 0, 0), make_real3(0, 0, 0)),
        real3x3(make_real3(2, -1, 0), make_real3(-1, 2, -1), make_real3(0, -1, 1))};
    mat.getData().copyFromHost(data1.data());

    //setup right hand side
    Vector3X           b(3);
    std::vector<real3> data2 = {
        make_real3(0.1, 0, 0), make_real3(0, 0, 0), make_real3(0, 0, -0.1)};
