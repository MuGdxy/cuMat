...
void testLUDecompositionReal()
{
    ...
    double dataA[2][5][5]{{{-0.509225, -0.713714, -0.376735, 1.50941, -1.51876},
    ...
    double dataAB[2][5][2]{{{0.179554, -0.504097},
                            {-0.524464, 0.0395376},
                            {0.794196, 0.678125},
                            {-0.432833, 1.02502},
                            {-0.67292, -0.228904}},
    ...
    double determinantData[2][1][1]{{{31.1144}}, {{-43.7003}}};
    mat_t  determinant =
        BMatrixXdR::fromArray(determinantData).cast<Scalar>().template block<1, 1, 2>(0, 0, 0);

    //perform LU decomposition
    LUDecomposition<mat_t>                           decomposition(A);
    typename LUDecomposition<mat_t>::EvaluatedMatrix matrixLU =
        decomposition.getMatrixLU();
...
#define I *cdouble(0, 1)
    ...
    cdouble                                            dataA[2][5][5]{
        ...
        {{-1.18698 + 1.57933 I, 1.4197 + 0.138538 I, -0.106781 + 0.603585 I, 1.70241 + 0.942044 I, -1.58697 - 1.22207 I},
                                                    ...
                                                    {0.951035 - 0.640323 I,
                                                     ...
                                                     -1.35505 - 1.60575 I}}};
    mat_t A =
        BMatrixXcdR::fromArray(dataA).cast<Scalar>().template block<5, 5, 2>(0, 0, 0);

    cdouble dataB[2][5][2]{{{-1.25871 + 1.09482 I, 0.914983 - 0.89768 I},
    ...
    mat_t   B =
        BMatrixXcdR::fromArray(dataB).cast<Scalar>().template block<5, 2, 2>(0, 0, 0);

    cdouble dataAB[2][5][2]{{{0.308061 - 1.45214 I, -0.299341 + 0.273846 I},
    ...
    mat_t   AB =
        BMatrixXcdR::fromArray(dataAB).cast<Scalar>().template block<5, 2, 2>(0, 0, 0);

    cdouble determinantData[2][1][1]{{{131.186 + 58.9019 I}}, {{-86.5874 + 23.9651 I}}};
    mat_t determinant =
        BMatrixXcdR::fromArray(determinantData).cast<Scalar>().template block<1, 1, 2>(0, 0, 0);
...
{
    typedef Matrix<Scalar, Dynamic, Dynamic, 2, Flags> mat_t;
    double dataA[2][5][5]{{{5.49558, -5.00076, -1.36761, -3.65355, -1.7765},
    ...
    mat_t  A =
        BMatrixXdR::fromArray(dataA).cast<Scalar>().template block<5, 5, 2>(0, 0, 0);
    double logDeterminantData[2][1][1]{{{6.875344150773254}}, {{7.55471060202439}}};
    mat_t logDeterminant =
        BMatrixXdR::fromArray(logDeterminantData).cast<Scalar>().template block<1, 1, 2>(0, 0, 0);
