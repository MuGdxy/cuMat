...
{
    int data[2][3][4] = {{{5, 2, 4, 1}, {8, 5, 6, 2}, {-5, 0, 3, -9}},
                         {{11, 0, 0, 3}, {-7, -2, -4, 1}, {9, 4, 7, -7}}};
    ...
    SECTION("host scalar")
    {
        int expected[2][3][4] = {{{10, 4, 8, 2}, {16, 10, 12, 4}, {-10, 0, 6, -18}},
                                 {{22, 0, 0, 6}, {-14, -4, -8, 2}, {18, 8, 14, -14}}};
    ...
    {
        int expected[2][3][4] = {{{10, 4, 8, 2}, {-8, -5, -6, -2}, {-5, 0, 3, -9}},
                                 {{22, 0, 0, 6}, {7, 2, 4, -1}, {9, 4, 7, -7}}};
        int       other[1][3][1] = {{{2}, {-1}, {1}}};
        VectorXiR s1             = VectorXiR::fromArray(other);
        VectorXiR s2 =
            s1.block<Dynamic, 1, 1>(0, 0, 0, s1.rows(), s1.cols(), s1.batches());
        //note: no operator to avoid ambiguity with matrix product
        assertMatrixEquality(expected, s1.cwiseMul(m1));
        assertMatrixEquality(expected, s1.cwiseMul(m2));
        assertMatrixEquality(expected, m1.cwiseMul(s1));
    ...
    SECTION("device row")
    {
        int expected[2][3][4] = {{{10, 2, 0, -2}, {16, 5, 0, -4}, {-10, 0, 0, 18}},
                                 {{22, 0, 0, -6}, {-14, -2, 0, -2}, {18, 4, 0, 14}}};
        int          other[1][1][4] = {{{2, 1, 0, -2}}};
        RowVectorXiR s1             = RowVectorXiR::fromArray(other);
        RowVectorXiR s2 =
            s1.block<1, Dynamic, 1>(0, 0, 0, s1.rows(), s1.cols(), s1.batches());
        //note: no operator to avoid ambiguity with matrix product
        assertMatrixEquality(expected, s1.cwiseMul(m1));
        assertMatrixEquality(expected, s1.cwiseMul(m2));
        assertMatrixEquality(expected, m1.cwiseMul(s1));
    ...
    SECTION("device batch")
    {
        ...
        BScalariC s2 =
            s1.block<1, 1, Dynamic>(0, 0, 0, s1.rows(), s1.cols(), s1.batches());
        //note: no operator to avoid ambiguity with matrix product
        assertMatrixEquality(expected, s1.cwiseMul(m1));
        assertMatrixEquality(expected, s1.cwiseMul(m2));
        assertMatrixEquality(expected, m1.cwiseMul(s1));
...
TEST_CASE("broadcasting-substract", "[binary]")
{
    ...
    SECTION("host scalar")
    {
        int expectedLeft[2][3][4] = {{{5, 8, 6, 9}, {2, 5, 4, 8}, {15, 10, 7, 19}},
                                     {{-1, 10, 10, 7}, {17, 12, 14, 9}, {1, 6, 3, 17}}};
        int expectedRight[2][3][4] = {
            {{-5, -8, -6, -9}, {-2, -5, -4, -8}, {-15, -10, -7, -19}},
            {{1, -10, -10, -7}, {-17, -12, -14, -9}, {-1, -6, -3, -17}}};
    ...
    SECTION("device column")
    {
        int expectedLeft[2][3][4] = {
            {{5, 8, 6, 9}, {-8, -5, -6, -2}, {-5, -10, -13, -1}},
            {{-1, 10, 10, 7}, {7, 2, 4, -1}, {-19, -14, -17, -3}}};
        int expectedRight[2][3][4] = {
            {{-5, -8, -6, -9}, {8, 5, 6, 2}, {5, 10, 13, 1}},
            {{1, -10, -10, -7}, {-7, -2, -4, 1}, {19, 14, 17, 3}}};
    ...
    {
        int expectedLeft[2][3][4] = {
            {{5, -2, -14, 0}, {2, -5, -16, -1}, {15, 0, -13, 10}},
            {{-1, 0, -10, -2}, {17, 2, -6, 0}, {1, -4, -17, 8}}};
        int expectedRight[2][3][4] = {{{-5, 2, 14, 0}, {-2, 5, 16, 1}, {-15, 0, 13, -10}},
                                      {{1, 0, 10, 2}, {-17, -2, 6, 0}, {-1, 4, 17, -8}}};
    ...
    SECTION("device batch")
    {
        int expectedLeft[2][3][4] = {
            {{5, 8, 6, 9}, {2, 5, 4, 8}, {15, 10, 7, 19}},
            {{-21, -10, -10, -13}, {-3, -8, -6, -11}, {-19, -14, -17, -3}}};
        int expectedRight[2][3][4] = {
            {{-5, -8, -6, -9}, {-2, -5, -4, -8}, {-15, -10, -7, -19}},
            {{21, 10, 10, 13}, {3, 8, 6, 11}, {19, 14, 17, 3}}};
