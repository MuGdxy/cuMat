...
TEST_CASE("fixed_block_rw", "[block]")
{
    ...
    {
        auto slice =
            cuMat::Matrix<int, rows, cols, 1, cuMat::ColumnMajor>::fromEigen(m_host1[i]);
    ...
    for(int i = 0; i < batches; ++i)
        REQUIRE(m_host1[i] == m_host2[i]);
    for(int i = 0; i < batches; ++i)
        REQUIRE(m_host1[i] == m_host3[i]);
}
...
TEST_CASE("dynamic_block_rw", "[block]")
{
    ...
    cuMat::Matrix<int, cuMat::Dynamic, cuMat::Dynamic, cuMat::Dynamic, cuMat::ColumnMajor> m_device(
        rows, cols, batches);
    for(int i = 0; i < batches; ++i)
    {
        auto slice =
            cuMat::Matrix<int, cuMat::Dynamic, cuMat::Dynamic, 1, cuMat::ColumnMajor>::fromEigen(
                m_host1[i]);
    ...
    const cuMat::Matrix<int, cuMat::Dynamic, cuMat::Dynamic, cuMat::Dynamic, cuMat::ColumnMajor> m_device_const =
        m_device;
...
TEST_CASE("cwise_vector", "[block]")
{
    SECTION("Row Vector")
    {
        ...
        REQUIRE(cuMat::internal::traits<decltype(fixedSegmentExpr)>::Flags
                == cuMat::internal::traits<cuMat::VectorXi>::Flags);

        auto dynamicSegmentExpr = vCwise.segment(1, 2);
        INFO(typeid(dynamicSegmentExpr).name());
        REQUIRE(cuMat::internal::traits<decltype(dynamicSegmentExpr)>::BatchesAtCompileTime
                == 1);
        REQUIRE(cuMat::internal::traits<decltype(dynamicSegmentExpr)>::RowsAtCompileTime == 1);
        REQUIRE(cuMat::internal::traits<decltype(dynamicSegmentExpr)>::ColsAtCompileTime
                == cuMat::Dynamic);
        REQUIRE(cuMat::internal::traits<decltype(dynamicSegmentExpr)>::Flags
                == cuMat::internal::traits<cuMat::VectorXi>::Flags);
