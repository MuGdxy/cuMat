...
TEST_CASE("Sparse -> Dense", "[Sparse]")
{
    SECTION("CSR")
    {
        ...
        smatrix.getData().slice(0) = VectorXf::fromEigen(
            (Eigen::VectorXf(9) << 1, 4, 2, 3, 5, 7, 8, 9, 6).finished());
        smatrix.getData().slice(1) = VectorXf::fromEigen(
            (Eigen::VectorXf(9) << -1, -4, -2, -3, -5, -7, -8, -9, -6).finished());
        ...
        float expected[2][4][5] = {
            {{1, 4, 0, 0, 0}, {0, 2, 3, 0, 0}, {5, 0, 0, 7, 8}, {0, 0, 9, 0, 6}},
            {{-1, -4, 0, 0, 0}, {0, -2, -3, 0, 0}, {-5, 0, 0, -7, -8}, {0, 0, -9, 0, -6}}};
    ...
    SECTION("CSC")
    {
        ...
        pattern.nnz  = 9;
        pattern.IA   = SPattern::IndexVector::fromEigen(
            (Eigen::VectorXi(9) << 0, 2, 0, 1, 1, 3, 2, 2, 3).finished());
        pattern.JA = SPattern::IndexVector::fromEigen(
            (Eigen::VectorXi(6) << 0, 2, 4, 6, 7, 9).finished());
        REQUIRE_NOTHROW(pattern.assertValid());
        SMatrix_t smatrix(pattern);
        smatrix.getData().slice(0) = VectorXf::fromEigen(
            (Eigen::VectorXf(9) << 1, 5, 4, 2, 3, 9, 7, 8, 6).finished());
        smatrix.getData().slice(1) = VectorXf::fromEigen(
            (Eigen::VectorXf(9) << -1, -5, -4, -2, -3, -9, -7, -8, -6).finished());
        INFO(smatrix);
    ...
    SECTION("ELLPACK")
    {
        ...
        REQUIRE_NOTHROW(pattern.assertValid());
        SMatrix_t smatrix(pattern);
        smatrix.getData().slice(0) = MatrixXf::fromEigen(
            (Eigen::MatrixXf(4, 3) << 1, 4, 666, 2, 3, 666, 5, 7, 8, 9, 6, 666).finished());
        smatrix.getData().slice(1) = MatrixXf::fromEigen(
            (Eigen::MatrixXf(4, 3) << -1, -4, 666, -2, -3, 666, -5, -7, -8, -9, -6, 666)
                .finished());
        INFO(smatrix);
...
TEST_CASE("Dense -> Sparse", "[Sparse]")
{
    SECTION("CSR")
    {
        ...
        float inputData[2][4][5] = {
            {{1, 4, 42, 42, 42}, {42, 2, 3, 42, 42}, {5, 42, 42, 7, 8}, {42, 42, 9, 42, 6}},
            {{-1, -4, 42, 42, 42}, {42, -2, -3, 42, 42}, {-5, 42, 42, -7, -8}, {42, 42, -9, 42, -6}}};
        ...
        //Check data array
        VectorXf batch1 = VectorXf::fromEigen(
            (Eigen::VectorXf(9) << 1, 4, 2, 3, 5, 7, 8, 9, 6).finished());
        VectorXf batch2 = VectorXf::fromEigen(
            (Eigen::VectorXf(9) << -1, -4, -2, -3, -5, -7, -8, -9, -6).finished());
    ...
    SECTION("ELLPACK")
    {
        ...
        //Check data array
        MatrixXf batch1 = MatrixXf::fromEigen(
            (Eigen::MatrixXf(4, 3) << 1, 4, 0, 2, 3, 0, 5, 7, 8, 9, 6, 0).finished());
        MatrixXf batch2 = MatrixXf::fromEigen(
            (Eigen::MatrixXf(4, 3) << -1, -4, 0, -2, -3, 0, -5, -7, -8, -9, -6, 0)
                .finished());
        MatrixXb mask = MatrixXb::fromEigen(
            (Eigen::Matrix<bool, Eigen::Dynamic, Eigen::Dynamic>(4, 3) << true,
             true,
             false,
             true,
             true,
...
TEST_CASE("Sparse-Cwise", "[Sparse]")
{
    ...
    float expectedDense[1][4][5] = {
        {{2, 16, 0, 0, 0}, {0, 5, 9, 0, 0}, {25, 0, 0 /*not 1*/, 49, 64}, {0, 0, 81, 0 /*not 1*/, 36}},
    };
...
TEST_CASE("Compound-SparseMatrix", "[Sparse]")
{
    ...
    pattern.nnz  = 4;
    pattern.IA =
        SPattern::IndexVector::fromEigen((Eigen::VectorXi(4) << 0, 1, 1, 2).finished());
    pattern.JA =
        SPattern::IndexVector::fromEigen((Eigen::VectorXi(3) << 0, 2, 4).finished());
    REQUIRE_NOTHROW(pattern.assertValid());
    SMatrix_t mat1(pattern);
    SMatrix_t mat2(pattern);
    mat1.getData().slice(0) =
        VectorXi::fromEigen((Eigen::VectorXi(4) << 1, 2, 3, 4).finished());
