...
void runLinearBenchmark()
{
    ...
    for(int N = minN; N <= maxN; N *= 2)
    {
        ...
        std::cout << "Benchmark, N=" << N << ", B=" << outerDim1 << "*"
                  << outerDim2 << std::endl;
        Timings t = {0};
    ...
    std::ofstream outStream(outputDir + "batched_reductions_"
                            + std::to_string(sizePower) + ".json");
    outStream << results;
    outStream.close();

    std::string launchParams =
        "\"" + pythonPath + " " + std::string(CUMAT_STR(PYTHON_FILES))
        + "MakePlotsLinear.py" + " \"" + outputDir + "batched_reductions_"
        + std::to_string(sizePower) + "\" " + "\"";
...
void runFullBenchmark()
{
    ...
    for(double i = std::log2(double(minNumBatches)); i <= std::log2(double(maxNumBatches));
        i += step)
    {
        int numBatches = static_cast<int>(std::round(std::pow(2, i)));
        for(double j = std::log2(double(minBatchSize));
            j <= std::log2(double(maxBatchSize));
            j += step)
        {
            cuMat::Index batchSize =
                static_cast<cuMat::Index>(std::round(std::pow(2, j)));
            ...
            if(set.count(tuple) > 0)
                continue;  //already processed
            set.insert(tuple);
            if(totalSize > maxTotalSize)
            {
                std::cout << "Run numBatches=" << numBatches << ", batchSize=" << batchSize
                          << " -> too large, skip" << std::endl;
                continue;
            }
            if(totalSize < minTotalSize)
            {
                std::cout << "Run numBatches=" << numBatches << ", batchSize=" << batchSize
                          << " -> too small, skip" << std::endl;
                continue;
            }

            std::cout << "Run numBatches=" << numBatches << ", batchSize=" << outerDim1
                      << "*" << outerDim2 << " (=" << totalSize << ")" << std::endl;
            ...
            t /= runs;
            outStreamRow << numBatches << "\t" << batchSize << "\t" << t.baseline
                         << "\t" << t.thread << "\t" << t.warp << "\t" << t.block64
                         << "\t" << t.block128 << "\t" << t.block256 << "\t"
                         << t.block512 << "\t" << t.block1024 << "\t" << t.device1
                         << "\t" << t.device2 << "\t" << t.device4 << "\t" << t.device8
                         << "\t" << t.device16 << "\t" << t.device32 << std::endl;
            ...
            t /= runs;
            outStreamCol << numBatches << "\t" << batchSize << "\t" << t.baseline
                         << "\t" << t.thread << "\t" << t.warp << "\t" << t.block64
                         << "\t" << t.block128 << "\t" << t.block256 << "\t"
                         << t.block512 << "\t" << t.block1024 << "\t" << t.device1
                         << "\t" << t.device2 << "\t" << t.device4 << "\t" << t.device8
            ...
            t /= runs;
            outStreamBatch
                << numBatches << "\t" << batchSize << "\t" << t.baseline << "\t"
                << t.thread << "\t" << t.warp << "\t" << t.block64 << "\t"
                << t.block128 << "\t" << t.block256 << "\t" << t.block512
                << "\t" << t.block1024 << "\t" << t.device1 << "\t" << t.device2
                << "\t" << t.device4 << "\t" << t.device8 << "\t" << t.device16
                << "\t" << t.device32 << std::endl;
        }
    }
    ...
    launchParams = "\"" + pythonPath + " " + std::string(CUMAT_STR(PYTHON_FILES))
                   + "MakePlotsFull.py" + " \"" + outputDir
                   + "batched_reductions_full_row" + "\"" + " Row " + "\"";
    std::cout << launchParams << std::endl;
    system(launchParams.c_str());
    launchParams = "\"" + pythonPath + " " + std::string(CUMAT_STR(PYTHON_FILES))
                   + "MakePlotsFull.py" + " \"" + outputDir
                   + "batched_reductions_full_col" + "\"" + " Column " + "\"";
    std::cout << launchParams << std::endl;
    system(launchParams.c_str());
    launchParams = "\"" + pythonPath + " " + std::string(CUMAT_STR(PYTHON_FILES))
                   + "MakePlotsFull.py" + " \"" + outputDir
                   + "batched_reductions_full_batch" + "\"" + " Batch " + "\"";
    std::cout << launchParams << std::endl;
    system(launchParams.c_str());
}
