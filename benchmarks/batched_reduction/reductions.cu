...
Timings benchmark(Index rows, Index cols, Index batches, bool compare, bool optimize)
{
    ...
    Mat target1(_Axis & Axis::Row ? 1 : rows,
                _Axis & Axis::Column ? 1 : cols,
                _Axis & Axis::Batch ? 1 : batches);
    Mat target2(_Axis & Axis::Row ? 1 : rows,
                _Axis & Axis::Column ? 1 : cols,
                _Axis & Axis::Batch ? 1 : batches);
    ...
    target2.setZero();
    start.record(stream);
    ReduceThread<functor::Sum<_Scalar>, _Scalar, _Axis>(
        source, target2, functor::Sum<_Scalar>(), _Scalar(0));
    end.record(stream);
    ...
    target2.setZero();
    start.record(stream);
    ReduceWarp<functor::Sum<_Scalar>, _Scalar, _Axis>(
        source, target2, functor::Sum<_Scalar>(), _Scalar(0));
    end.record(stream);
    if(compare && !static_cast<bool>(((target1 - target2) <= _Scalar(1e-5)).all()))
    {
    ...
    target2.setZero();
    start.record(stream);
    ReduceBlock<functor::Sum<_Scalar>, _Scalar, _Axis, 64>(
        source, target2, functor::Sum<_Scalar>(), _Scalar(0));
    end.record(stream);
    if(compare && !static_cast<bool>(((target1 - target2) <= _Scalar(1e-5)).all()))
    {
    ...
    target2.setZero();
    start.record(stream);
    ReduceBlock<functor::Sum<_Scalar>, _Scalar, _Axis, 128>(
        source, target2, functor::Sum<_Scalar>(), _Scalar(0));
    end.record(stream);
    if(compare && !static_cast<bool>(((target1 - target2) <= _Scalar(1e-5)).all()))
    {
    ...
    target2.setZero();
    start.record(stream);
    ReduceBlock<functor::Sum<_Scalar>, _Scalar, _Axis, 256>(
        source, target2, functor::Sum<_Scalar>(), _Scalar(0));
    end.record(stream);
    if(compare && !static_cast<bool>(((target1 - target2) <= _Scalar(1e-5)).all()))
    {
    ...
    target2.setZero();
    start.record(stream);
    ReduceBlock<functor::Sum<_Scalar>, _Scalar, _Axis, 512>(
        source, target2, functor::Sum<_Scalar>(), _Scalar(0));
    end.record(stream);
    if(compare && !static_cast<bool>(((target1 - target2) <= _Scalar(1e-5)).all()))
    {
    ...
    target2.setZero();
    start.record(stream);
    ReduceBlock<functor::Sum<_Scalar>, _Scalar, _Axis, 1024>(
        source, target2, functor::Sum<_Scalar>(), _Scalar(0));
    end.record(stream);
    if(compare && !static_cast<bool>(((target1 - target2) <= _Scalar(1e-5)).all()))
    {
    ...
    else
    {
        ...
        target2.setZero();
        start.record(stream);
        ReduceDevice<functor::Sum<_Scalar>, _Scalar, _Axis, 1>::run(
            source, target2, functor::Sum<_Scalar>(), _Scalar(0));
        end.record(stream);
        if(compare && !static_cast<bool>(((target1 - target2) <= _Scalar(1e-5)).all()))
        {
        ...
        target2.setZero();
        start.record(stream);
        ReduceDevice<functor::Sum<_Scalar>, _Scalar, _Axis, 2>::run(
            source, target2, functor::Sum<_Scalar>(), _Scalar(0));
        end.record(stream);
        if(compare && !static_cast<bool>(((target1 - target2) <= _Scalar(1e-5)).all()))
        {
        ...
        target2.setZero();
        start.record(stream);
        ReduceDevice<functor::Sum<_Scalar>, _Scalar, _Axis, 4>::run(
            source, target2, functor::Sum<_Scalar>(), _Scalar(0));
        end.record(stream);
        if(compare && !static_cast<bool>(((target1 - target2) <= _Scalar(1e-5)).all()))
        {
        ...
        target2.setZero();
        start.record(stream);
        ReduceDevice<functor::Sum<_Scalar>, _Scalar, _Axis, 8>::run(
            source, target2, functor::Sum<_Scalar>(), _Scalar(0));
        end.record(stream);
        if(compare && !static_cast<bool>(((target1 - target2) <= _Scalar(1e-5)).all()))
        {
        ...
        target2.setZero();
        start.record(stream);
        ReduceDevice<functor::Sum<_Scalar>, _Scalar, _Axis, 16>::run(
            source, target2, functor::Sum<_Scalar>(), _Scalar(0));
        end.record(stream);
        if(compare && !static_cast<bool>(((target1 - target2) <= _Scalar(1e-5)).all()))
        {
        ...
        target2.setZero();
        start.record(stream);
        ReduceDevice<functor::Sum<_Scalar>, _Scalar, _Axis, 32>::run(
            source, target2, functor::Sum<_Scalar>(), _Scalar(0));
        end.record(stream);
        if(compare && !static_cast<bool>(((target1 - target2) <= _Scalar(1e-5)).all()))
        {
...
void benchmarkAndPrint(Index rows, Index cols, Index batches, bool compare)
{
    ...
    std::cout << "Type=" << typeid(_Scalar).name() << ", Axis="
              << (_Axis == Axis::Row ? "Row" : (_Axis == Axis::Column ? "Column" : "Batch"))
