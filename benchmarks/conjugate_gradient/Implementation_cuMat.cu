...
#define IDX(x, y) ((y) + (x)*gridSize)
        ...
        pattern.JA   = SPattern::IndexVector(matrixSize + 1);
        pattern.JA.copyFromHost(matrix.outerIndexPtr());
        pattern.IA = SPattern::IndexVector(pattern.nnz);
        pattern.IA.copyFromHost(matrix.innerIndexPtr());
        pattern.assertValid();
        SMatrix mat(pattern);
