...
static const char* getErrorName(cusparseStatus_t status)
{
    switch(status)
    {
        case CUSPARSE_STATUS_NOT_INITIALIZED:
            return "CUSPARSE_STATUS_NOT_INITIALIZED: cuSPARSE was not initialized";
        case CUSPARSE_STATUS_ALLOC_FAILED:
            return "CUSPARSE_STATUS_ALLOC_FAILED: resource allocation failed";
        case CUSPARSE_STATUS_INVALID_VALUE:
            return "CUSPARSE_STATUS_INVALID_VALUE: invalid value was passed as argument";
        case CUSPARSE_STATUS_ARCH_MISMATCH:
            return "CUSPARSE_STATUS_ARCH_MISMATCH: device architecture not supported";
        case CUSPARSE_STATUS_MAPPING_ERROR:
            return "CUSPARSE_STATUS_MAPPING_ERROR: access to GPU memory failed";
        case CUSPARSE_STATUS_EXECUTION_FAILED:
            return "CUSPARSE_STATUS_EXECUTION_FAILED: general kernel launch failure";
        case CUSPARSE_STATUS_INTERNAL_ERROR:
            return "CUSPARSE_STATUS_INTERNAL_ERROR: an internal error occured";
        case CUSPARSE_STATUS_MATRIX_TYPE_NOT_SUPPORTED:
            return "CUSPARSE_STATUS_MATRIX_TYPE_NOT_SUPPORTED: functionality is not supported";
        case CUSPARSE_STATUS_ZERO_PIVOT:
            return "CUSPARSE_STATUS_ZERO_PIVOT: pivot is zero";
        default:
            return "";
    }
}
static void cusparseSafeCall(cusparseStatus_t status, const char* file, const int line)
{
    ...
    {
        std::string msg = cuMat::internal::ErrorHelpers::format(
            "cusparseSafeCall() failed at %s:%i : %s\n", file, line, getErrorName(status));
...
void benchmark_cuBlas(const std::vector<std::string>& parameterNames,
...
#define IDX(x, y) ((y) + (x)*gridSize)
        ...
        for(int x = 0; x < gridSize; ++x)
            for(int y = 0; y < gridSize; ++y)
            {
                int row = IDX(x, y);
                if(x > 0)
                    matrix.insert(row, IDX(x - 1, y)) = -1;
                if(y > 0)
                    matrix.insert(row, IDX(x, y - 1)) = -1;
        ...
        cuMat::VectorXi JA(matrixSize + 1);
        JA.copyFromHost(matrix.outerIndexPtr());
        cuMat::VectorXi IA(nnz);
        IA.copyFromHost(matrix.innerIndexPtr());
        cuMat::VectorXf data(nnz);
        data.copyFromHost(matrix.valuePtr());
        ...
        for(int run = 0; run < runs; ++run)
        {
            ...
            {
                CUSPARSE_SAFE_CALL(cusparseScsrmv(handle,
