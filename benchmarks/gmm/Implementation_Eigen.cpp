...
class Gaussian
{
  ...
  public:
    ...
    void computeDetInv()
    {
        ...
        if(n_ == 1)
        {
            covariance_.block<1, 1>(0, 0).computeInverseAndDetWithCheck(
                invCovariance_, det, invertible);
            logdet_ = std::log(det);
        }
        else if(n_ == 2)
        {
            covariance_.block<2, 2>(0, 0).computeInverseAndDetWithCheck(
                invCovariance_, det, invertible);
            logdet_ = std::log(det);
        }
        else if(n_ == 3)
        {
            covariance_.block<3, 3>(0, 0).computeInverseAndDetWithCheck(
                invCovariance_, det, invertible);
            logdet_ = std::log(det);
        }
        else if(n_ == 4)
        {
            covariance_.block<4, 4>(0, 0).computeInverseAndDetWithCheck(
                invCovariance_, det, invertible);
            logdet_ = std::log(det);
        }
        else
        {
            ...
            logdet_ =
                cholesky_.matrixL().toDenseMatrix().diagonal().array().log().sum();
        }
        if(!invertible)
        {
...
#pragma omp parallel for
        ...
        for(int k = 0; k < components; ++k)
        {
            ...
            for(int i = 0; i < numPoints; ++i)
                gaussians[k].cov() +=
                    std::exp(logW(i, k)) * (points.col(i) - gaussians[k].mean())
                    * (points.col(i) - gaussians[k].mean()).transpose();
