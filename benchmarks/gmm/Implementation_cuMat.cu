...
class Gaussian
{
  ...
  public:
    ...
    void computeDetInv()
    {
        ...
        {
            std::cerr << "Covariance is singular, re-initialize with the identity matrix"
                      << std::endl;
    ...
    cuMat::BScalard logP(const cuMat::BVectorXd& x)
    {
        ...
        else
        {
            auto alpha = tmp_.dot(
                cholesky_
                    .solve(tmp_.swapAxis<cuMat::Row, cuMat::Batch, cuMat::NoAxis>())
                    .swapAxis<cuMat::Row, cuMat::NoAxis, cuMat::Column>());
            return -0.5 * (alpha + (n_ * log2pi + logdet_));
        }
    }
...
cuMat::Matrix<Scalar, cuMat::Dynamic, 1, 1, cuMat::ColumnMajor> LSE_row(
    const cuMat::MatrixBase<Derived>& vec)
{
    auto m = vec.template maxCoeff<cuMat::Axis::Column>().eval();
    return m + (vec - m).cwiseExp().template sum<cuMat::Axis::Column>().cwiseLog();
}
//Log-Sum-Exp, reduction per column
template <typename Derived, typename Scalar = typename Derived::Scalar>
cuMat::Matrix<Scalar, 1, cuMat::Dynamic, 1, cuMat::ColumnMajor> LSE_col(
    const cuMat::MatrixBase<Derived>& vec)
{
    auto m = vec.template maxCoeff<cuMat::Axis::Row>().eval();
    return m + (vec - m).cwiseExp().template sum<cuMat::Axis::Row>().cwiseLog();
}
}  // namespace
                     ...
                     Json::Object&      returnValues)
{
    ...
    {  //points
        ...
        for(int i = 0; i < components * (1 + dimension + dimension * dimension); ++i)
            in >> dummy;
        ...
        for(int i = 0; i < components; ++i)
        {
            ...
            for(int x = 0; x < dimension; ++x)
                for(int y = 0; y < dimension; ++y)
                    in >> cov(x, y);
            gaussians.emplace_back(cuMat::VectorXd::fromEigen(mean),
    ...
    {
        std::cout << "\nComponent " << k << "\nWeight: "
                  << std::exp(static_cast<double>(logWeights.slice(k))) << "\n"
    ...
    for(int iter = 0; iter < numIterations; ++iter)
    {
        ...
        {
            logW.col(k) =
                gaussians[k].logP(points).swapAxis<cuMat::Batch, cuMat::NoAxis, cuMat::NoAxis>()
                + logWeights.slice(k);
        }
        ...
        logWeights.inplace() =
            lseC.swapAxis<cuMat::NoAxis, cuMat::NoAxis, cuMat::Column>()
            - std::log(numPoints);
        for(int k = 0; k < components; ++k)
        {
#ifndef NDEBUG
            ...
            gaussians[k].cov().inplace() =
                (logW.col(k)
                     .cwiseExp()
                     .swapAxis<cuMat::NoAxis, cuMat::NoAxis, cuMat::Row>()
                     .cwiseMul((points - gaussians[k].mean())
    ...
    {
        Json::Object com;
        com.Insert(std::make_pair("Weight",
                                  std::exp(static_cast<double>(logWeights.slice(k)))));
        Json::Array m, c;
