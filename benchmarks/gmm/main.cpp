...
int main(int argc, char* argv[])
{
    ...
    for(auto it = sets.Begin(); it != sets.End(); ++it)
    {
        ...
        launchParams =
            "\"" + pythonPath + " " + numpyFile + " " + std::to_string(dimension) + " "
            + std::to_string(numTrueComponents) + " " + std::to_string(numPoints)
            + " " + std::to_string(++seed) + " GroundTruth.txt \"";
        std::cout << "  Args: " << launchParams << std::endl;
        exec(launchParams.c_str());
        numpyFile    = std::string(CUMAT_STR(PYTHON_FILES)) + "GenerateData.py";
        launchParams = "\"" + pythonPath + " " + numpyFile + " "
                       + std::to_string(dimension) + " " + std::to_string(numTrueComponents)
                       + " 0" + " " + std::to_string(++seed) + " Initial.txt \"";
        std::cout << "  Args: " << launchParams << std::endl;
        exec(launchParams.c_str());
        ...
        launchParams =
            ...
            + "MakePlots1.py" + " GroundTruth.txt Initial.txt " + " \"" + outputDir
            + "GMM_Test.json" + "\" " + std::string(CUMAT_STR(CONFIG_FILE)) + "\"";
    ...
    for(auto it = benchmarks.Begin(); it != benchmarks.End(); ++it)
    {
        ...
        for(int run = 0; run < runs; ++run)
        {
            ...
            launchParams = "\"" + pythonPath + " " + numpyFile + " "
                           ...
                           + std::to_string(numTrueComponents) + " 0" + " "
                           + std::to_string(++seed) + " Initial.txt \"";
            std::cout << "  Args: " << launchParams << std::endl;
            exec(launchParams.c_str());
