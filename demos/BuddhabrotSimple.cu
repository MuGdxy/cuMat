...
struct BuddhaBrotFunctor
{
  ...
  public:
    BuddhaBrotFunctor(MatrixXi buddhabrot,
                      int      width,
                      int      height,
                      double   range,
                      double   offsetReal,
    ...
    __device__ CUMAT_STRONG_INLINE ReturnType operator()(Index row, Index col, Index batch) const
    {
        ...
        const cdouble c((row - width_ * 0.5) / step_ + offsetReal_,
...
int main()
{
    ...
    MatrixXi mandelbrot =
        MatrixXi::NullaryExpr<BuddhaBrotFunctor>(WIDTH, HEIGHT, 1, functor);
...
void writePGM(const MatrixXi& intensities, const char const* filename)
{
    ...
    MatrixXf normalizedIntensitites =
        intensities.cast<float>()
