...
//output / helper
void writePPM(const OutputMatrix& red,
              const OutputMatrix& green,
              const OutputMatrix& blue,
...
struct BuddhaBrotFunctor
{
  ...
  public:
    BuddhaBrotFunctor(OutputMatrix    buddhabrot,
    ...
    __device__ CUMAT_STRONG_INLINE ReturnType operator()(cdouble z, Index row, Index col, Index batch) const
    {
        ...
        const cdouble c((x + samplingPattern[msaa].x - width_ * 0.5) / step_ + offsetReal_,
...
int main()
{
    ...
    for(int i = 0; i < 3; ++i)
    {
        ...
        //first pass
        BuddhaBrotFunctor<true> functor1(
            buddhabrot[i], iterationMatrix, WIDTH, HEIGHT, RANGE, OFFSET_REAL, ITERATIONS_PER_STEP, ESCAPE_RADIUS_SQ);
        ...
        BuddhaBrotFunctor<false> functor2(
            buddhabrot[i], iterationMatrix, WIDTH, HEIGHT, RANGE, OFFSET_REAL, ITERATIONS_PER_STEP, ESCAPE_RADIUS_SQ);
        for(int j = 0; j < iterations; j += ITERATIONS_PER_STEP)
        {
            //print and sync at every few kernel calls
              ...
              const char const*   filename)
{
    ...
    Matrix<uint8_t, Dynamic, Dynamic, 1, ColumnMajor> byteRed =
        ((red.cast<float>() / static_cast<float>(static_cast<int>(red.maxCoeff()))) * 255)
            .cast<uint8_t>();
    Matrix<uint8_t, Dynamic, Dynamic, 1, ColumnMajor> byteGreen =
        ((green.cast<float>() / static_cast<float>(static_cast<int>(green.maxCoeff()))) * 255)
            .cast<uint8_t>();
    Matrix<uint8_t, Dynamic, Dynamic, 1, ColumnMajor> byteBlue =
        ((blue.cast<float>() / static_cast<float>(static_cast<int>(blue.maxCoeff()))) * 255)
            .cast<uint8_t>();
