...
{
  protected:
    CUMAT_STRONG_INLINE _DecompositionImpl& impl()
    {
        return *static_cast<_DecompositionImpl*>(this);
    }
    CUMAT_STRONG_INLINE const _DecompositionImpl& impl() const
    {
        return *static_cast<const _DecompositionImpl*>(this);
    }
  ...
  public:
        ...
        , rhs_(rhs.derived())
    {
        ...
        CUMAT_ASSERT(CUMAT_IMPLIES(_Solver::Batches != 1,
                     ...
                     && "batch size of the matrix and the right hand side does not match");
        CUMAT_ASSERT(decomposition.rows() == decomposition.cols()
                     && "matrix must be square");  //TODO: relax for Least-Squares problems
        CUMAT_ASSERT(decomposition.cols() == rhs.rows()
                     && "matrix size does not match right-hand-side");
    }
