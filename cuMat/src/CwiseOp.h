...
#define CUMAT_DECLARE_ASSIGNMENT_HANDLER(mode, op1, op2)                                                                     \
    ...
    {                                                                                                                        \
        static __device__ CUMAT_STRONG_INLINE void assign(                                                                   \
            M& matrix, V value, Index index, std::integral_constant<int, AccessFlags::RWCwiseRef>) /*reference access*/      \
        {                                                                                                                    \
            matrix.rawCoeff(index) op1 value;                                                                                \
        }                                                                                                                    \
        static __device__ CUMAT_STRONG_INLINE void assign(                                                                   \
            M& matrix, V value, Index index, std::integral_constant<int, AccessFlags::RWCwise>) /*read-write access*/        \
        {                                                                                                                    \
        ...
        static __device__ CUMAT_STRONG_INLINE void assign(                                                                   \
            ...
            V     value,                                                                                                     \
            Index index,                                                                                                     \
            std::integral_constant<int, AccessFlags::RWCwiseRef | AccessFlags::RWCwise>) /*both is possible, use reference*/ \
        {                                                                                                                    \
            matrix.rawCoeff(index) op1 value;                                                                                \
        }                                                                                                                    \
        static __device__ CUMAT_STRONG_INLINE void assign(M& matrix, V value, Index index)                                   \
        {                                                                                                                    \
            assign(matrix,                                                                                                   \
                   value,                                                                                                    \
                   index,                                                                                                    \
                   std::integral_constant<int, traits<M>::AccessFlags&(AccessFlags::RWCwiseRef | AccessFlags::RWCwise)>());  \
        }                                                                                                                    \
    };
...
struct Assignment<_Dst, _Src, _Mode, DenseDstTag, CwiseSrcTag>
{
    static void assign(_Dst& dst, const _Src& src)
    {
        ...
        CUMAT_LOG_DEBUG("Evaluate component wise expression "
                        << typeid(src.derived()).name() << "\n rows=" << src.rows()
                        << ", cols=" << src.cols() << ", batches=" << src.batches());

        //here is now the real logic
        Context&           ctx = Context::current();
        KernelLaunchConfig cfg = ctx.createLaunchConfig1D(
            static_cast<unsigned int>(dst.size()),
            kernels::CwiseEvaluationKernel<SrcActual, DstActual, _Mode>);
        kernels::CwiseEvaluationKernel<SrcActual, DstActual, _Mode>
