...
struct ProductAssignment<_Dst, SparseDstTag, _DstOp, _SrcLeft, CwiseSrcTag, _SrcLeftOp, _SrcRight, CwiseSrcTag, _SrcRightOp, _AssignmentMode>
{
    ...
    static void assign(_Dst& dst, const Op& op)
    {
        ...
        Assignment<_Dst, Op, _AssignmentMode, typename traits<_Dst>::DstTag, CwiseSrcTag>::assign(
            dst, op);
    }
};
...
#pragma unroll
        for(int b = 0; b < Batches; ++b)
        {
            internal::CwiseAssignmentHandler<M, OutputScalar, Mode>::assign(
                output, value[b], outer + b * output.rows());
        }
...
template <typename _Dst,  // ProductArgOp _DstOp,
          typename _SrcLeftScalar,
          int _SrcLeftBatches,  // ProductArgOp _SrcLeftOp,
          typename _SrcRight,   // ProductArgOp _SrcRightOp,
          AssignmentMode _AssignmentMode>
struct ProductAssignment<_Dst, DenseDstTag, ProductArgOp::NONE, SparseMatrix<_SrcLeftScalar, _SrcLeftBatches, SparseFlags::CSR>, CwiseSrcTag, ProductArgOp::NONE, _SrcRight, CwiseSrcTag, ProductArgOp::NONE, _AssignmentMode>
{
    using SrcLeft = SparseMatrix<_SrcLeftScalar, _SrcLeftBatches, SparseFlags::CSR>;
    using Op =
        ProductOp<SrcLeft, _SrcRight, ProductArgOp::NONE, ProductArgOp::NONE, ProductArgOp::NONE>;
    using Scalar = typename Op::Scalar;
    ...
    static void assign(_Dst& dst, const Op& op)
    {
        ...
        CUMAT_LOG_DEBUG("Evaluate SparseMatrix-DenseVector multiplication "
                        << typeid(op.derived()).name()
                        << " matrix rows=" << op.derived().left().rows()
                        << ", cols=" << op.left().cols());
        ;
        ...
        kernels::CSRMVKernel_StaticBatches<SrcLeft, typename _SrcRight::Type, DstActual, _AssignmentMode, Op::Batches>
            <<<cfg.block_count, cfg.thread_per_block, 0, ctx.stream()>>>(
                cfg.virtual_size,
                op.derived().left().derived(),
                op.derived().right().derived(),
                dst.derived());
        CUMAT_CHECK_ERROR();
...
//Currently, only non-batched CSR matrices are supported
//TODO: support also CSC, vector on the left, transposed and conjugated versions
template <typename _Dst, typename _SrcLeftChild, typename _SrcRight, AssignmentMode _AssignmentMode>
struct ProductAssignment<_Dst, DenseDstTag, ProductArgOp::NONE, SparseExpressionOp<_SrcLeftChild, SparseFlags::CSR>, CwiseSrcTag, ProductArgOp::NONE, _SrcRight, CwiseSrcTag, ProductArgOp::NONE, _AssignmentMode>
{
    ...
    static void assign(_Dst& dst, const Op& op)
    {
        ...
        Context&           ctx = Context::current();
        KernelLaunchConfig cfg = ctx.createLaunchConfig1D(
            static_cast<unsigned int>(dst.size()),
            kernels::CSRMVKernel_StaticBatches<SrcLeft, typename _SrcRight::Type, DstActual, _AssignmentMode, Op::Batches>);
        kernels::CSRMVKernel_StaticBatches<SrcLeft, typename _SrcRight::Type, DstActual, _AssignmentMode, Op::Batches>
            <<<cfg.block_count, cfg.thread_per_block, 0, ctx.stream()>>>(
                cfg.virtual_size,
...
namespace kernels
{
    ...
    template <typename L,
              ...
              bool BroadcastMatrix = internal::traits<L>::BatchesAtCompileTime == 1,
              bool BroadcastRhs = internal::traits<R>::BatchesAtCompileTime == 1>
    __global__ void ELLPACKMVKernel_StaticBatches(dim3    virtual_size,
                                                  const L matrix,
                                                  const R vector,
                                                  M       output)
    {
        ...
        typedef typename M::Scalar OutputScalar;
        typedef ProductElementFunctor<LeftScalar, RightScalar, ProductArgOp::NONE, ProductArgOp::NONE, ProductArgOp::NONE> Functor;
        const SparsityPattern<SparseFlags::ELLPACK>::IndexMatrix& indices =
            matrix.getSparsityPattern().indices;
        ...
        for(int ci = 0; ci < nnzPerRow; ++ci)
        {
            ...
            if(col < 0)
                continue;  //TODO: test if it is faster to continue reading (and set col=0) and discard before assignment
#pragma unroll
            for(int b = 0; b < Batches; ++b)
            {
                LeftScalar tmp1 =
...
#pragma unroll
        ...
        {
            internal::CwiseAssignmentHandler<M, OutputScalar, Mode>::assign(
                output, value[b], row + b * output.rows());
        }
        CUMAT_KERNEL_1D_LOOP_END
    }
...
template <typename _Dst,  // ProductArgOp _DstOp,
          ...
          typename _SrcRight,   // ProductArgOp _SrcRightOp,
          AssignmentMode _AssignmentMode>
struct ProductAssignment<_Dst, DenseDstTag, ProductArgOp::NONE, SparseMatrix<_SrcLeftScalar, _SrcLeftBatches, SparseFlags::ELLPACK>, CwiseSrcTag, ProductArgOp::NONE, _SrcRight, CwiseSrcTag, ProductArgOp::NONE, _AssignmentMode>
{
...
//TODO: support also CSC, vector on the left, transposed and conjugated versions
template <typename _Dst, typename _SrcLeftChild, typename _SrcRight, AssignmentMode _AssignmentMode>
struct ProductAssignment<_Dst, DenseDstTag, ProductArgOp::NONE, SparseExpressionOp<_SrcLeftChild, SparseFlags::ELLPACK>, CwiseSrcTag, ProductArgOp::NONE, _SrcRight, CwiseSrcTag, ProductArgOp::NONE, _AssignmentMode>
{
