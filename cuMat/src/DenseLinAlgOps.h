...
namespace kernels
{
    ...
    __device__ CUMAT_STRONG_INLINE DeviceMatrix<Scalar, Dims, Flags> loadMat(
        const Matrix<Scalar, Rows, Cols, Batches, Flags>& mat, Index index)
    {
        const DeviceMatrix<Scalar, Dims, Flags>* data =
            reinterpret_cast<DeviceMatrix<Scalar, Dims, Flags>*>(mat.data());
        return data[index];
    }
    template <int Dims, typename T, typename Scalar = typename internal::traits<T>::Scalar, int Flags = internal::traits<T>::Flags>
    ...
    template <int Dims, typename Scalar, int Rows, int Cols, int Batches, int Flags>
    __device__ CUMAT_STRONG_INLINE void storeMat(Matrix<Scalar, Rows, Cols, Batches, Flags>& mat,
                                                 const DeviceMatrix<Scalar, Dims, Flags>& out,
                                                 Index index)
    {
        DeviceMatrix<Scalar, Dims, Flags>* data =
            reinterpret_cast<DeviceMatrix<Scalar, Dims, Flags>*>(mat.data());
        data[index] = out;
    }
    template <int Dims, typename T, typename Scalar = typename internal::traits<T>::Scalar, int Flags = internal::traits<T>::Flags>
    __device__ CUMAT_STRONG_INLINE void storeMat(T& mat,
                                                 const DeviceMatrix<Scalar, Dims, Flags>& out,
                                                 Index index)
    {
        out.store(mat, index);
    ...
    struct DeterminantFunctor<Scalar, 3, Input>
    {
        static __device__ CUMAT_STRONG_INLINE Scalar run(const Input& mat)
        {
            return mat.m00 * mat.m11 * mat.m22 + mat.m01 * mat.m12 * mat.m20
                   + mat.m02 * mat.m10 * mat.m21 - mat.m02 * mat.m11 * mat.m20
                   - mat.m01 * mat.m10 * mat.m22 - mat.m00 * mat.m12 * mat.m21;
        }
    };
    template <typename Scalar, typename Input>
    struct DeterminantFunctor<Scalar, 4, Input>
    {
        static __device__ CUMAT_STRONG_INLINE Scalar run(const Input& mat)
        {
            ...
            Scalar fDet = fA0 * fB5 - fA1 * fB4 + fA2 * fB3 + fA3 * fB2
                          - fA4 * fB1 + fA5 * fB0;
            return fDet;
        }
    };
    template <typename T,
              ...
              int TFlags      = internal::traits<T>::Flags,
              int MFlags      = internal::traits<M>::Flags>
    __global__ void DeterminantKernel(dim3 virtual_size, const T expr, M matrix)
    {
        ...
        Scalar det =
            DeterminantFunctor<Scalar, Dims, DeviceMatrix<Scalar, Dims, TFlags>>::run(in);
    ...
    struct InverseFunctor<Scalar, 3, Input, Output>
    {
        static __device__ CUMAT_STRONG_INLINE Output run(const Input& mat, Scalar& det)
        {
            det = mat.m00 * mat.m11 * mat.m22 + mat.m01 * mat.m12 * mat.m20
                  + mat.m02 * mat.m10 * mat.m21 - mat.m02 * mat.m11 * mat.m20
                  - mat.m01 * mat.m10 * mat.m22 - mat.m00 * mat.m12 * mat.m21;
            Scalar f = Scalar(1) / det;
    ...
    template <typename MatIn,
              ...
              int InFlags     = internal::traits<MatIn>::Flags,
              int OutFlags    = internal::traits<MatOut>::Flags>
...
struct traits<DeterminantOp<_Child>>
{
    ...
    enum
    {
        ...
        InputSize = internal::traits<_Child>::RowsAtCompileTime
...
struct traits<InverseOp<_Child>>
{
    ...
    enum
    {
        HasExplicitFormula = internal::traits<_Child>::RowsAtCompileTime >= 1
                             && internal::traits<_Child>::RowsAtCompileTime <= 4,
        ...
        Flags = RequiresColumnMajor ? CUMAT_NAMESPACE Flags::ColumnMajor :
                                      internal::traits<_Child>::Flags,
  ...
  private:
    ...
    static void evalImpl(const Op& op, Derived& m, std::integral_constant<int, 1>)
    {
        ...
        KernelLaunchConfig cfg = ctx.createLaunchConfig1D(
            m.batches(), internal::kernels::DeterminantKernel<Child_wrapped, Derived, 1>);
        internal::kernels::DeterminantKernel<Child_wrapped, Derived, 1>
            <<<cfg.block_count, cfg.thread_per_block, 0, ctx.stream()>>>(
                cfg.virtual_size, inWrapped, m);
        CUMAT_CHECK_ERROR();
    }
    ...
    static void evalImpl(const Op& op, Derived& m, std::integral_constant<int, 2>)
    {
        ...
        Context&           ctx = Context::current();
        KernelLaunchConfig cfg = ctx.createLaunchConfig1D(
            m.batches(), internal::kernels::DeterminantKernel<Child_wrapped, Derived, 2>);
        internal::kernels::DeterminantKernel<Child_wrapped, Derived, 2>
            <<<cfg.block_count, cfg.thread_per_block, 0, ctx.stream()>>>(
                cfg.virtual_size, inWrapped, m);
        CUMAT_CHECK_ERROR();
    }
    ...
    static void evalImpl(const Op& op, Derived& m, std::integral_constant<int, 3>)
    {
        ...
        Context&           ctx = Context::current();
        KernelLaunchConfig cfg = ctx.createLaunchConfig1D(
            m.batches(), internal::kernels::DeterminantKernel<Child_wrapped, Derived, 3>);
        internal::kernels::DeterminantKernel<Child_wrapped, Derived, 3>
            <<<cfg.block_count, cfg.thread_per_block, 0, ctx.stream()>>>(
                cfg.virtual_size, inWrapped, m);
        CUMAT_CHECK_ERROR();
    }
    ...
    static void evalImpl(const Op& op, Derived& m, std::integral_constant<int, 4>)
    {
        ...
        Context&           ctx = Context::current();
        KernelLaunchConfig cfg = ctx.createLaunchConfig1D(
            m.batches(), internal::kernels::DeterminantKernel<Child_wrapped, Derived, 4>);
        internal::kernels::DeterminantKernel<Child_wrapped, Derived, 4>
            <<<cfg.block_count, cfg.thread_per_block, 0, ctx.stream()>>>(
                cfg.virtual_size, inWrapped, m);
        CUMAT_CHECK_ERROR();
    }
    ...
    static void evalImpl(const Op& op, Derived& m, std::integral_constant<int, DynamicSize>)
    {
        ...
        else
        {
            ...
            internal::Assignment<Derived, DetType, AssignmentMode::ASSIGN, typename Derived::DstTag, typename DetType::SrcTag>::assign(
                m.derived(), lu.determinant());
            //CUMAT_SAFE_CALL(cudaDeviceSynchronize());
        }
    }
...
struct Assignment<_Dst, _Src, _AssignmentMode, _DstTag, InverseSrcTag>
{
  private:
    ...
    static void evalImpl(const Op& op, Derived& m, std::integral_constant<int, 1>)
    {
        ...
        Context&           ctx = Context::current();
        KernelLaunchConfig cfg = ctx.createLaunchConfig1D(
            m.batches(), internal::kernels::InverseKernel<Child_wrapped, Derived, 1>);
        internal::kernels::InverseKernel<Child_wrapped, Derived, 1>
            <<<cfg.block_count, cfg.thread_per_block, 0, ctx.stream()>>>(
                cfg.virtual_size, inWrapped, m);
        CUMAT_CHECK_ERROR();
    }
    ...
    static void evalImpl(const Op& op, Derived& m, std::integral_constant<int, 2>)
    {
        ...
        Context&           ctx = Context::current();
        KernelLaunchConfig cfg = ctx.createLaunchConfig1D(
            m.batches(), internal::kernels::InverseKernel<Child_wrapped, Derived, 2>);
        internal::kernels::InverseKernel<Child_wrapped, Derived, 2>
            <<<cfg.block_count, cfg.thread_per_block, 0, ctx.stream()>>>(
                cfg.virtual_size, inWrapped, m);
        CUMAT_CHECK_ERROR();
    }
    ...
    static void evalImpl(const Op& op, Derived& m, std::integral_constant<int, 3>)
    {
        ...
        Context&           ctx = Context::current();
        KernelLaunchConfig cfg = ctx.createLaunchConfig1D(
            m.batches(), internal::kernels::InverseKernel<Child_wrapped, Derived, 3>);
        internal::kernels::InverseKernel<Child_wrapped, Derived, 3>
            <<<cfg.block_count, cfg.thread_per_block, 0, ctx.stream()>>>(
                cfg.virtual_size, inWrapped, m);
        CUMAT_CHECK_ERROR();
    }
    ...
    static void evalImpl(const Op& op, Derived& m, std::integral_constant<int, 4>)
    {
        ...
        Context&           ctx = Context::current();
        KernelLaunchConfig cfg = ctx.createLaunchConfig1D(
            m.batches(), internal::kernels::InverseKernel<Child_wrapped, Derived, 4>);
        internal::kernels::InverseKernel<Child_wrapped, Derived, 4>
            <<<cfg.block_count, cfg.thread_per_block, 0, ctx.stream()>>>(
                cfg.virtual_size, inWrapped, m);
        CUMAT_CHECK_ERROR();
    }
    ...
    static void evalImpl(const Op& op, Derived& m, std::integral_constant<int, DynamicSize>)
    {
        ...
        else
        {
            ...
            internal::Assignment<Derived, InverseType, AssignmentMode::ASSIGN, typename Derived::DstTag, typename InverseType::SrcTag>::assign(
                m.derived(), lu.inverse());
            //CUMAT_SAFE_CALL(cudaDeviceSynchronize());
        }
    }
