...
struct SparsityPattern<SparseFlags::CSC>
{
    ...
    */
    ...
    using DataMatrix = Matrix<_Scalar, Dynamic, 1, _Batches, Flags::ColumnMajor>;
...
struct SparsityPattern<SparseFlags::ELLPACK>
{
	...
	*/
    ...
    template <typename _Scalar, int _Batches>
    using DataMatrix = Matrix<_Scalar, Dynamic, Dynamic, _Batches, Flags::ColumnMajor>;
    ...
    * \return the number of non-zero entries
    */
    __host__ __device__ CUMAT_STRONG_INLINE Index getNNZ() const
    {
        return rows * nnzPerRow;
    }
    /**
    * \brief Allocates the data matrix for this sparsity type with the specified number of batches.
    */
...
struct SparseMatrixIndexEvaluator
{
    ...
    */
    static __device__ void linearToCoords(
        const Sparsity& sparsity, Index index, Index& row, Index& col, Index& batch);
    /**
...
class SparseMatrixBase : public MatrixBase<_Derived>
{
  ...
  public:
    ...
    */
    __host__ __device__ CUMAT_STRONG_INLINE Index nnz() const
    {
        return sparsity_.getNNZ() * batches_;
    }
    ...
    */
     ...
     * \return 
     */
    __device__ CUMAT_STRONG_INLINE const Scalar& getSparseCoeff(Index row,
                                                                Index col,
                                                                Index batch,
                                                                Index index) const
    {
