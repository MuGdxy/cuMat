...
class LUDecomposition : public DecompositionBase<LUDecomposition<_MatrixType>>
{
  public:
    ...
    enum
    {
        ...
        InputIsMatrix =
            std::is_same<_MatrixType, Matrix<Scalar, Rows, Columns, Batches, Flags>>::value
    };
  ...
  public:
    ...
    */
    void compute(const MatrixBase<_MatrixType>& matrix, bool inplace = false)
    {
        ...
        const int m = internal::narrow_cast<int>(
            Transposed ? decompositedMatrix_.cols() : decompositedMatrix_.rows());
        const int n = internal::narrow_cast<int>(
            Transposed ? decompositedMatrix_.rows() : decompositedMatrix_.cols());
        ...
        for(Index batch = 0; batch < batches; ++batch)
        {
            internal::CusolverApi::current().cusolverGetrf(
                m,
                n,
                internal::CusolverApi::cast(decompositedMatrix_.data() + batch * m * n),
                lda,
        ...
        for(Index i = 0; i < batches; ++i)
        {
            if(singular_[i] < 0)
            {
                throw cuda_error(internal::ErrorHelpers::format(
                    "Getrf failed at batch %d, parameter %d was invalid", i, -singular_[i]));
            }
        }
    }

    CUMAT_STRONG_INLINE Index rows() const
    {
        return decompositedMatrix_.rows();
    }
    CUMAT_STRONG_INLINE Index cols() const
    {
        return decompositedMatrix_.cols();
    }
    CUMAT_STRONG_INLINE Index batches() const
    {
        return decompositedMatrix_.batches();
    }

    const EvaluatedMatrix& getMatrixLU() const { return decompositedMatrix_; }

    const PivotArray& getPivots() const { return pivots_; }
     ...
     */
    bool isSingular(Index batch = 0) const { return singular_[batch] > 0; }
    ...
    DeterminantMatrix determinant() const
    {
        ...
        return decompositedMatrix_.diagonal()
            .template prod<Axis::Row | Axis::Column>()  //multiply diagonal elements
            .cwiseMul(UnaryOp<PivotArray, PermutationSignFunctor>(pivots_, PermutationSignFunctor())
                          .template prod<Axis::Row | Axis::Column>()
                          .template cast<Scalar>()  //compute sign of the permutation
            );
    }
    ...
    */
    ...
    void _solve_impl(const MatrixBase<_RHS>& rhs, MatrixBase<_Target>& target) const
    {
        ...
        for(Index i = 0; i < batches(); ++i)
        {
            if(isSingular(i))
                throw cuda_error(internal::ErrorHelpers::format(
                    "Batch %d is singular, can't solve the system", i));
        }
        ...
        internal::Assignment<_Target, const _RHS, AssignmentMode::ASSIGN, typename _Target::DstTag, typename _RHS::SrcTag>::assign(
            target.derived(), rhs.derived());
        ...
        for(Index batch = 0; batch < batches; ++batch)
        {
            internal::CusolverApi::current().cusolverGetrs(
                ...
                n,
                nrhs,
                internal::CusolverApi::cast(A + batch * strideA),
                lda,
                devIpiv + batch * n,
                internal::CusolverApi::cast(B + batch * strideB),
                ldb,
                devInfo.data() + batch);
        }
        ...
        for(Index i = 0; i < batches; ++i)
        {
            ...
            {
                throw cuda_error(internal::ErrorHelpers::format(
                    "Getrs failed at batch %d, parameter %d was invalid", i, -hostInfo[i]));
            }
        }
    }
};
