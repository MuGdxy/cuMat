...
template <typename _Scalar, int _Batches, int _SparseFlags>
class SparseMatrix
    : public SparseMatrixBase<SparseMatrix<_Scalar, _Batches, _SparseFlags>>
{
    CUMAT_STATIC_ASSERT(_SparseFlags == SparseFlags::CSR || _SparseFlags == SparseFlags::CSC
                            || _SparseFlags == SparseFlags::ELLPACK,
  ...
  public:
	...
	* \return the data vector of the non-zero entries
	*/
    __host__ __device__ CUMAT_STRONG_INLINE const DataMatrix& getData() const
    {
        return A_;
    }
    ...
    */
    ...
    {
        Index linear = internal::SparseMatrixIndexEvaluator<_SparseFlags>::coordsToLinear(
            sparsity_, row, col, batch);
        if(linear >= 0)
    ...
    */
    ...
    {
        return internal::SparseMatrixIndexEvaluator<_SparseFlags>::linearToCoords(
            sparsity_, index, row, col, batch);
    }
    ...
    * \return
    */
    __device__ CUMAT_STRONG_INLINE const _Scalar& getSparseCoeff(Index /*row*/,
                                                                 Index /*col*/,
                                                                 Index /*batch*/,
                                                                 Index index) const
    {
    ...
    */
    ...
    {
        Type mat(cloneSparsity ? getSparsityPattern().deepClone() : getSparsityPattern(),
                 batches());
        mat.A_ = A_.deepClone();
        return mat;
    }
  ...
  public:
	...
	*/
    ...
    CUMAT_STRONG_INLINE Type& operator=(const MatrixBase<Derived>& expr)
    {
        ...
        checkInitialized();
        makeExclusiveUse();
        internal::Assignment<Type, Derived, AssignmentMode::ASSIGN, internal::SparseDstTag, typename internal::traits<Derived>::SrcTag>::assign(
            *this, expr.derived());
        return *this;
    }
...
#define CUMAT_COMPOUND_ASSIGNMENT(op, mode)                                                                                                    \
    ...
    */                                     \
    ...
    {                                                                                                                                          \
        ...
        checkInitialized();                                                                                                                    \
        internal::Assignment<Type, Derived, AssignmentMode::mode, internal::SparseDstTag, typename internal::traits<Derived>::SrcTag>::assign( \
            *this, expr.derived());                                                                                                            \
        return *this;                                                                                                                          \
    }
...
#undef CUMAT_COMPOUND_ASSIGNMENT
    ...
    */
    ...
    CUMAT_STRONG_INLINE T& operator*=(const S& scalar)
    {
        ...
        internal::Assignment<Type, Expr, AssignmentMode::MUL, internal::SparseDstTag, typename internal::traits<Expr>::SrcTag>::assign(
...
__host__ std::ostream& operator<<(std::ostream& os,
                                  const SparseMatrix<_Scalar, _Batches, SparseFlags::CSR>& m)
{
    ...
    io::IOFormat fmt;
    os << " Outer Indices (row): ";
    io::print_matrix(os, m.getSparsityPattern().JA, fmt, true);
    os << " Inner Indices (column): ";
    io::print_matrix(os, m.getSparsityPattern().IA, fmt, true);
    os << " Data:\n";
    fmt.rowPrefix      = "   ";
    fmt.batchPrefix    = "  [\n";
    fmt.batchSuffix    = "\n  ]";
...
template <typename _Scalar, int _Batches>
__host__ std::ostream& operator<<(std::ostream& os,
                                  const SparseMatrix<_Scalar, _Batches, SparseFlags::CSC>& m)
{
    os << "SparseMatrix: " << std::endl;
    os << " rows=" << m.rows();
    os << ", cols=" << m.cols();
    ...
    os << " Outer Indices (column): " << m.getSparsityPattern().JA.toEigen().transpose()
       << std::endl;
    os << " Inner Indices (row): " << m.getSparsityPattern().IA.toEigen().transpose()
       << std::endl;
    for(int batch = 0; batch < m.batches(); ++batch)
    {
        os << " Data (Batch " << batch
           << "): " << m.getData().slice(batch).eval().toEigen().transpose()
           << std::endl;
    }
    return os;
}
...
template <typename _Scalar, int _Batches>
__host__ std::ostream& operator<<(std::ostream& os,
                                  const SparseMatrix<_Scalar, _Batches, SparseFlags::ELLPACK>& m)
{
    os << "SparseMatrix: " << std::endl;
    os << " rows=" << m.rows();
    os << ", cols=" << m.cols();
    ...
    {
        os << " Data (Batch " << batch
           << "): " << m.getData().slice(batch).eval().toEigen() << std::endl;
    }
    return os;
}
...
class SparseMatrixInplaceAssignment
{
  ...
  public:
    SparseMatrixInplaceAssignment(_SparseMatrix* matrix)
        : matrix_(matrix)
    {
    }
    ...
    CUMAT_STRONG_INLINE _SparseMatrix& operator=(const MatrixBase<Derived>& expr)
    {
        ...
        Assignment<_SparseMatrix, Derived, AssignmentMode::ASSIGN, SparseDstTag, typename Derived::SrcTag>::assign(
...
class SparseMatrixDirectAccess
    : public MatrixBase<SparseMatrixDirectAccess<_SparseMatrix>>
{
  public:
  ...
  public:
    SparseMatrixDirectAccess(const _SparseMatrix* matrix)
        : matrix_(*matrix)
    {
    }  //store by value, needed for the host->device transfer
