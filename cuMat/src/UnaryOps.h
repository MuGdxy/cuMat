...
class UnaryOp : public CwiseOp<UnaryOp<_Child, _UnaryFunctor>>
{
  ...
  public:
    explicit UnaryOp(const MatrixBase<_Child>& child,
                     const _UnaryFunctor&      functor = _UnaryFunctor())
...
#define DEFINE_GENERAL_FUNCTOR(Name, Fn)                                       \
    ...
    {                                                                          \
      public:                                                                  \
        ...
        __device__ CUMAT_STRONG_INLINE _Scalar operator()(const _Scalar& x,    \
                                                          Index          row,  \
                                                          Index          col,  \
                                                          Index batch) const   \
        {                                                                      \
...
#define DEFINE_FUNCTOR(Name, Scalar, Fn)                                       \
    ...
    {                                                                          \
      public:                                                                  \
        ...
        __device__ CUMAT_STRONG_INLINE Scalar operator()(const Scalar& x,      \
                                                         Index         row,    \
                                                         Index         col,    \
                                                         Index batch) const    \
        {                                                                      \
...
DEFINE_GENERAL_FUNCTOR(cwiseInverse,
                       (CUMAT_NAMESPACE internal::NumTraits<_Scalar>::RealType(1) / x));
...
class CastingOp : public CwiseOp<CastingOp<_Child, _Target>>
{
  ...
  public:
    ...
    __device__ CUMAT_STRONG_INLINE Scalar coeff(Index row, Index col, Index batch, Index index) const
    {
        return functor::CastFunctor<SourceType, TargetType>::cast(
            child_.derived().coeff(row, col, batch, index));
    }
};
...
struct traits<AsDiagonalOp<_Child>>
{
    ...
    {
        Flags             = internal::traits<_Child>::Flags,
        Size              = internal::traits<_Child>::RowsAtCompileTime == 1 ?
...
class ExtractComplexPartOp<_Child, _Imag, false>
    : public CwiseOp<ExtractComplexPartOp<_Child, _Imag, false>>
{
  public:
...
*/
template <typename _Child, bool _Imag>
class ExtractComplexPartOp<_Child, _Imag, true>
    : public CwiseOp<ExtractComplexPartOp<_Child, _Imag, true>>
{
  public:
...
{
    CUMAT_STATIC_ASSERT(_Row == Axis::NoAxis || _Row == Axis::Row
                            || _Row == Axis::Column || _Row == Axis::Batch,
                        "_Row parameter must be either NoAxis, Row, Column or Batch");
    CUMAT_STATIC_ASSERT(_Col == Axis::NoAxis || _Col == Axis::Row
                            || _Col == Axis::Column || _Col == Axis::Batch,
                        "_Col parameter must be either NoAxis, Row, Column or Batch");
    CUMAT_STATIC_ASSERT(_Batch == Axis::NoAxis || _Batch == Axis::Row
                            || _Batch == Axis::Column || _Batch == Axis::Batch,
  ...
  public:
    ...
    __device__ CUMAT_STRONG_INLINE Scalar coeff(Index row, Index col, Index batch, Index index) const
    {
        return child_.derived().coeff(_Row == Axis::Row   ? row :
                                      _Col == Axis::Row   ? col :
...
#define UNARY_OP(Name, Op)                                                                                                                         \
    ...
    {                                                                                                                                              \
        ...
        return CUMAT_NAMESPACE                                                                                                                     \
            UnaryOp<_Derived, CUMAT_NAMESPACE functor::UnaryMathFunctor_##Op<typename CUMAT_NAMESPACE internal::traits<_Derived>::Scalar>>(        \
                mat.derived());                                                                                                                    \
    }
...
template <typename _Child>
CUMAT_NAMESPACE ExtractComplexPartOp<_Child, false, false> real(
    const CUMAT_NAMESPACE MatrixBase<_Child>& mat)
{
...
template <typename _Child>
CUMAT_NAMESPACE ExtractComplexPartOp<_Child, true, false> imag(
    const CUMAT_NAMESPACE MatrixBase<_Child>& mat)
{
