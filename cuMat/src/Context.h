...
#define CUMAT_KERNEL_3D_LOOP(i, j, k, virtual_size)                            \
    ...
    {                                                                          \
        ...
        CUMAT_NAMESPACE Index j =                                              \
            (__i - (k * virtual_size.x * virtual_size.y)) / virtual_size.x;    \
...
class Context
{
  ...
  public:
        ...
        , stream_(nullptr)
    {
        ...
        CUMAT_LOG_DEBUG("Context initialized for thread 0x"
    ...
    ~Context()
    {
        ...
        CUMAT_LOG_DEBUG("Context deleted for thread 0x"
                        << std::hex << std::this_thread::get_id());
#if CUMAT_CONTEXT_DEBUG_MEMORY == 1
    ...
    */
    ...
    KernelLaunchConfig createLaunchConfig1D(Index size, T func) const
    {
        ...
        CUMAT_LOG_DEBUG("Best potential occupancy for "
                        << typeid(T).name() << " found to be: blocksize=" << bestBlockSize
                        << ", gridSize=" << minGridSize);
        KernelLaunchConfig cfg = {dim3(size_, 1, 1),
	...
	*/
    ...
    KernelLaunchConfig createLaunchConfig2D(unsigned int sizex, unsigned int sizey, T func) const
    {
        ...
        CUMAT_LOG_DEBUG("Best potential occupancy for "
                        << typeid(T).name() << " found to be: blocksize=" << bestBlockSize
                        << ", gridSize=" << minGridSize);
        KernelLaunchConfig cfg = {dim3(sizex, sizey, 1),
                                  dim3(bestBlockSize, 1, 1),
                                  dim3(minGridSize, 1, 1)};
        return cfg;
    }
	...
	*/
    template <class T>
    KernelLaunchConfig createLaunchConfig3D(unsigned int sizex,
                                            unsigned int sizey,
                                            unsigned int sizez,
                                            T            func) const
    {
        ...
        CUMAT_LOG_DEBUG("Best potential occupancy for "
                        << typeid(T).name() << " found to be: blocksize=" << bestBlockSize
                        << ", gridSize=" << minGridSize);
        KernelLaunchConfig cfg = {dim3(sizex, sizey, sizez),
                                  dim3(bestBlockSize, 1, 1),
                                  dim3(minGridSize, 1, 1)};
        return cfg;
    }
