...
class BinaryOp : public CwiseOp<BinaryOp<_Left, _Right, _BinaryFunctor>>
{
  ...
  public:
    BinaryOp(const MatrixBase<_Left>&  left,
             const MatrixBase<_Right>& right,
             const _BinaryFunctor&     functor = _BinaryFunctor())
        : left_(left.derived())
        , right_(right.derived())
        , functor_(functor)
    {
        CUMAT_STATIC_ASSERT((std::is_same<typename internal::traits<_Left>::Scalar,
                                          typename internal::traits<_Right>::Scalar>::value),
        ...
        CUMAT_STATIC_ASSERT(!(RowsLeft > 1 && RowsRight > 1) || (RowsLeft == RowsRight),
        ...
        CUMAT_STATIC_ASSERT(!(ColumnsLeft > 1 && ColumnsRight > 1)
                                || (ColumnsLeft == ColumnsRight),
                            "matrix sizes don't match");
    ...
    {
        return left_.derived().coeff(BroadcastRowsLeft ? 0 : row,
    ...
    {
        return right_.derived().coeff(BroadcastRowsRight ? 0 : row,
    ...
    {
        return functor_(
            getLeft(row, col, batch, linear), getRight(row, col, batch, linear), row, col, batch);
    }
};
...
#define DEFINE_GENERAL_FUNCTOR(Name, Fn)                                                 \
    ...
    {                                                                                    \
      public:                                                                            \
        ...
        __device__ CUMAT_STRONG_INLINE _Scalar operator()(                               \
            const _Scalar& x, const _Scalar& y, Index row, Index col, Index batch) const \
        {                                                                                \
            return Fn;                                                                   \
        }                                                                                \
    }
...
#define DEFINE_FUNCTOR(Name, Scalar, Fn)                                               \
    ...
    {                                                                                  \
      public:                                                                          \
        ...
        __device__ CUMAT_STRONG_INLINE Scalar operator()(                              \
            const Scalar& x, const Scalar& y, Index row, Index col, Index batch) const \
        {                                                                              \
            return Fn;                                                                 \
        }                                                                              \
    }
...
#define DEFINE_FALLBACK_FUNCTOR(Name)                                          \
    ...
    {                                                                          \
      public:                                                                  \
        CUMAT_STATIC_ASSERT((!std::is_same<_Scalar, _Scalar>::value),          \
                            "Functor not available for the selected type");    \
    }
...
#define BINARY_OP_SCALAR(Name, Op)                                                                               \
    template <typename _Left,                                                                                    \
              ...
              typename T = typename std::enable_if<                                                              \
                  CUMAT_NAMESPACE internal::canBroadcast<_Left, S>::value,                                       \
    ...
    {                                                                                                            \
        ...
        return CUMAT_NAMESPACE BinaryOp<CUMAT_NAMESPACE HostScalar<S>, _Right, CUMAT_NAMESPACE Op<S>>(           \
            CUMAT_NAMESPACE make_host_scalar<S>(left), right);                                                   \
    }                                                                                                            \
    template <typename _Left,                                                                                    \
              typename _Right,                                                                                   \
              typename S = typename CUMAT_NAMESPACE internal::traits<_Left>::Scalar,                             \
              typename T = typename std::enable_if<                                                              \
                  CUMAT_NAMESPACE internal::canBroadcast<_Right, S>::value,                                      \
    ...
    {                                                                                                            \
        CUMAT_ERROR_IF_NO_NVCC(binaryOp)                                                                         \
        return CUMAT_NAMESPACE BinaryOp<_Left, CUMAT_NAMESPACE HostScalar<S>, CUMAT_NAMESPACE Op<S>>(            \
            left, CUMAT_NAMESPACE make_host_scalar<S>(right));                                                   \
    }
#define BINARY_OP(Name, Op)                                                                                                           \
    template <typename _Left, typename _Right>                                                                                        \
    CUMAT_NAMESPACE BinaryOp<_Left, _Right, CUMAT_NAMESPACE Op<typename CUMAT_NAMESPACE internal::traits<_Left>::Scalar>> Name(       \
        const CUMAT_NAMESPACE MatrixBase<_Left>& left,                                                                                \
        const CUMAT_NAMESPACE MatrixBase<_Right>& right)                                                                              \
    {                                                                                                                                 \
        CUMAT_ERROR_IF_NO_NVCC(binaryOp)                                                                                              \
        return CUMAT_NAMESPACE BinaryOp<_Left, _Right, CUMAT_NAMESPACE Op<typename CUMAT_NAMESPACE internal::traits<_Left>::Scalar>>( \
            left, right);                                                                                                             \
    }                                                                                                                                 \
...
#define DEFINE_GENERAL_FUNCTOR(Name, Fn)                                                 \
    ...
    {                                                                                    \
      public:                                                                            \
        ...
        __device__ CUMAT_STRONG_INLINE bool operator()(                                  \
            const _Scalar& x, const _Scalar& y, Index row, Index col, Index batch) const \
        {                                                                                \
            return Fn;                                                                   \
        }                                                                                \
...
#define BINARY_OP_SCALAR(Name, Op)                                                                                                                \
    ...
    {                                                                                                                                             \
        ...
        return CUMAT_NAMESPACE BinaryOp<CUMAT_NAMESPACE HostScalar<S>, _Right, CUMAT_NAMESPACE Op<S>>(                                            \
            CUMAT_NAMESPACE make_host_scalar<S>(S(left)), right);                                                                                 \
    }                                                                                                                                             \
    template <typename _Left,                                                                                                                     \
              typename _Right,                                                                                                                    \
              typename S = typename CUMAT_NAMESPACE internal::traits<_Left>::Scalar,                                                              \
    ...
    {                                                                                                                                             \
        CUMAT_ERROR_IF_NO_NVCC(logicOp)                                                                                                           \
        return CUMAT_NAMESPACE BinaryOp<_Left, CUMAT_NAMESPACE HostScalar<S>, CUMAT_NAMESPACE Op<S>>(                                             \
            left, CUMAT_NAMESPACE make_host_scalar<S>(S(right)));                                                                                 \
    }
#define BINARY_OP(Name, Op)                                                                                                           \
    template <typename _Left, typename _Right>                                                                                        \
    CUMAT_NAMESPACE BinaryOp<_Left, _Right, CUMAT_NAMESPACE Op<typename CUMAT_NAMESPACE internal::traits<_Left>::Scalar>> Name(       \
