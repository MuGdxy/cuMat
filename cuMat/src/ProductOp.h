...
struct ProductElementFunctor
{
    ...
    static CUMAT_STRONG_INLINE __device__ Scalar mult(const _LeftScalar&  left,
                                                      const _RightScalar& right)
    {
        return left * right;
    }
};
...
class ProductOp
    : public MatrixBase<ProductOp<_Left, _Right, _OpLeft, _OpRight, _OpOutput>>
{
  public:
    ...
    enum
    {
        ...
        TransposedOutput =
            int(_OpOutput) & int(internal::ProductArgOp::TRANSPOSED) ? true : false,
    ...
    using LeftType =
        typename std::conditional<IsOuterProduct, left_wrapped_t, typename _Left::Type>::type;
    using RightType =
        typename std::conditional<IsOuterProduct, right_wrapped_t, typename _Right::Type>::type;
  ...
  public:
        ...
        , right_(right.derived())
    {
        ...
        CUMAT_STATIC_ASSERT(CUMAT_IMPLIES((ColumnsLeft >= 1 && RowsRight >= 1),
                                          (TransposedLeft ? RowsLeft : ColumnsLeft)
                                              == (TransposedRight ? ColumnsRight : RowsRight)),
        ...
        CUMAT_STATIC_ASSERT(!(BatchesLeft > 1 && BatchesRight > 1)
                                || (BatchesLeft == BatchesRight),
  ...
  public:
    ...
    */
    ...
    CUMAT_STRONG_INLINE __device__ Scalar coeff(Index row, Index col, Index batch, Index index) const
    {
        ...
        LeftScalar                                          left =
            left_.coeff(TransposedLeft ? 0 : r,
                        TransposedLeft ? r : 0,
        ...
        RightScalar                                          right =
            right_.coeff(TransposedRight ? c : 0,
                         TransposedRight ? 0 : c,
        ...
        //output
        Scalar output =
            internal::ProductElementFunctor<LeftScalar, RightScalar, _OpLeft, _OpRight, _OpOutput>::mult(
                left, right);
...
namespace internal
{
template <typename _Dst, typename _DstTag, ProductArgOp _DstOp, typename _SrcLeft, typename _SrcLeftTag, ProductArgOp _SrcLeftOp, typename _SrcRight, typename _SrcRightTag, ProductArgOp _SrcRightOp, AssignmentMode _AssignmentMode>
...
struct Assignment<_Dst, _Src, _AssignmentMode, _DstTag, ProductSrcTag>
{
    static void assign(_Dst& dst, const _Src& src)
    {
        ...
        ProductAssignment<typename _Dst::Type,
                          _DstTag,
                          internal::ProductArgOp(SrcActual::OutputOp),
                          typename SrcActual::LeftType,
                          typename traits<typename SrcActual::LeftType>::SrcTag,
                          internal::ProductArgOp(SrcActual::LeftOp),
                          typename SrcActual::RightType,
                          typename traits<typename SrcActual::RightType>::SrcTag,
                          internal::ProductArgOp(SrcActual::RightOp),
                          _AssignmentMode>::assign(dst.derived(), src.derived());
    }
};
...
template <typename _Dst, ProductArgOp _DstOp, typename _SrcLeft, ProductArgOp _SrcLeftOp, typename _SrcRight, ProductArgOp _SrcRightOp, AssignmentMode _AssignmentMode>
struct ProductAssignment<_Dst, DenseDstTag, _DstOp, _SrcLeft, CwiseSrcTag, _SrcLeftOp, _SrcRight, CwiseSrcTag, _SrcRightOp, _AssignmentMode>
{
    ...
    static void evalImpl(_Dst&                  mat,
                         float                  betaIn,
                         const left_wrapped_t&  left,
                         const right_wrapped_t& right,
                         const Op&              op,
                         std::integral_constant<bool, true> /*direct-write*/)
    {
           ...
           || (Op::TransposedOutput && CUMAT_IS_ROW_MAJOR(traits<_Dst>::Flags)))
        {
            ...
            A = left.data();
            B = right.data();
            transA = (Op::TransposedLeft == CUMAT_IS_COLUMN_MAJOR(Op::FlagsLeft)) ?
                         CUBLAS_OP_T :
                         CUBLAS_OP_N;
            transB = (Op::TransposedRight == CUMAT_IS_COLUMN_MAJOR(Op::FlagsRight)) ?
                         CUBLAS_OP_T :
                         CUBLAS_OP_N;
        ...
        else
        {
            ...
            A = right.data();
            B = left.data();
            transA = (Op::TransposedRight == CUMAT_IS_COLUMN_MAJOR(Op::FlagsRight)) ?
                         CUBLAS_OP_N :
                         CUBLAS_OP_T;
            transB = (Op::TransposedLeft == CUMAT_IS_COLUMN_MAJOR(Op::FlagsLeft)) ?
                         CUBLAS_OP_N :
                         CUBLAS_OP_T;
        ...
        if(Op::Batches > 1 || op.batches() > 1)
        {
            ...
            internal::CublasApi::current().cublasGemmBatched(
                ...
                n,
                k,
                internal::CublasApi::cast(&alpha),
                internal::CublasApi::cast(A),
                lda,
                strideA,
                internal::CublasApi::cast(B),
                ldb,
                strideB,
                internal::CublasApi::cast(&beta),
                internal::CublasApi::cast(C),
                ldc,
                strideC,
        ...
        else
        {
            ...
            internal::CublasApi::current().cublasGemm(transA,
                                                      transB,
                                                      m,
                                                      n,
                                                      k,
                         ...
                         std::integral_constant<bool, false> /*non-direct-write*/)
    {
        ...
        ProductAssignment<DstTmp, DenseDstTag, _DstOp, _SrcLeft, CwiseSrcTag, _SrcLeftOp, _SrcRight, CwiseSrcTag, _SrcRightOp, AssignmentMode::ASSIGN>::evalImpl(
            tmp, betaIn, left, right, op, std::integral_constant<bool, true>());
    ...
    static void assign(_Dst& dst, const Op& op)
    {
        ...
        //TODO: Handle different assignment modes
        static_assert((_AssignmentMode == AssignmentMode::ASSIGN
                       || _AssignmentMode == AssignmentMode::ADD),
                      "Matrix multiplication only supports the following assignment modes: =, +=");
...
ProductOp<_Left, _Right, internal::ProductArgOp::TRANSPOSED, internal::ProductArgOp::TRANSPOSED, internal::ProductArgOp::NONE> operator*(
    const TransposeOp<_Left, false>& left, const TransposeOp<_Right, false>& right)
{
    return ProductOp<_Left, _Right, internal::ProductArgOp::TRANSPOSED, internal::ProductArgOp::TRANSPOSED, internal::ProductArgOp::NONE>(
        left.getUnderlyingMatrix(), right.getUnderlyingMatrix());
}
...
*/
template <typename _Left, typename _Right>
ProductOp<_Left, _Right, internal::ProductArgOp::NONE, internal::ProductArgOp::TRANSPOSED, internal::ProductArgOp::NONE> operator*(
    const MatrixBase<_Left>& left, const TransposeOp<_Right, false>& right)
{
    return ProductOp<_Left, _Right, internal::ProductArgOp::NONE, internal::ProductArgOp::TRANSPOSED, internal::ProductArgOp::NONE>(
        left, right.getUnderlyingMatrix());
}
...
*/
template <typename _Left, typename _Right>
ProductOp<_Left, _Right, internal::ProductArgOp::TRANSPOSED, internal::ProductArgOp::NONE, internal::ProductArgOp::NONE> operator*(
    const TransposeOp<_Left, false>& left, const MatrixBase<_Right>& right)
{
    return ProductOp<_Left, _Right, internal::ProductArgOp::TRANSPOSED, internal::ProductArgOp::NONE, internal::ProductArgOp::NONE>(
        left.getUnderlyingMatrix(), right);
}
...
*/
template <typename _Left, typename _Right>
ProductOp<_Left, _Right, internal::ProductArgOp::NONE, internal::ProductArgOp::NONE, internal::ProductArgOp::NONE> operator*(
    const MatrixBase<_Left>& left, const MatrixBase<_Right>& right)
{
    return ProductOp<_Left, _Right, internal::ProductArgOp::NONE, internal::ProductArgOp::NONE, internal::ProductArgOp::NONE>(
        left, right);
}
