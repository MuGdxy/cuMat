...
struct ReductionEvaluatorHelper<_Input, _Output, Axis::Row>
{
    ...
    {
        return StridedMatrixInputIterator<_Input>(
            in, thrust::make_tuple(1, in.rows(), in.rows() * in.cols()));
    }
...
struct ReductionEvaluatorHelper<_Input, _Output, Axis::Column>
{
    ...
    {
        return StridedMatrixInputIterator<_Input>(
            in, thrust::make_tuple(in.cols(), 1, in.rows() * in.cols()));
    }
    static StridedMatrixOutputIterator<_Output> iterOut(_Output& out)
    {
...
struct ReductionEvaluatorHelper<_Input, _Output, Axis::Batch>
{
    ...
    {
        bool isRowMajor = CUMAT_IS_ROW_MAJOR(internal::traits<_Output>::Flags);
        return StridedMatrixOutputIterator<_Output>(
            out,
            isRowMajor ? thrust::make_tuple(out.cols(), Index(1), 1) :
...
struct ReductionEvaluatorHelper<_Input, _Output, Axis::Row | Axis::Batch>
{
    ...
    {
        return StridedMatrixInputIterator<_Input>(
            in, thrust::make_tuple(1, in.rows() * in.batches(), in.rows()));
    }
    static typename _Output::Scalar* iterOut(_Output& out)
    {
        return out.data();
...
struct ReductionEvaluatorHelper<_Input, _Output, Axis::Column | Axis::Batch>
{
    ...
    {
        return StridedMatrixInputIterator<_Input>(
            in, thrust::make_tuple(in.cols() * in.batches(), 1, in.cols()));
    }
    static typename _Output::Scalar* iterOut(_Output& out)
    {
        return out.data();
...
struct ReductionEvaluator<_Input, _Output, _Axis, _Op, _Scalar, ReductionAlg::Segmented>
{
    static void eval(const MatrixBase<_Input>& in, _Output& out, const _Op& op, const _Scalar& initial)
    {
        ...
        int numEntries = static_cast<int>(
            ReductionEvaluatorHelper<_Input, _Output, _Axis>::numEntries(in));
        int numBatches = static_cast<int>(
            ReductionEvaluatorHelper<_Input, _Output, _Axis>::numBatches(in));

        //call cub
        CountingInputIterator<int> iterOffsets(0, numEntries);
        Context&                   ctx                = Context::current();
        size_t                     temp_storage_bytes = 0;
        CUMAT_SAFE_CALL(cub::DeviceSegmentedReduce::Reduce(NULL,
                                                           temp_storage_bytes,
                                                           iterIn,
                                                           iterOut,
                                                           numBatches,
        ...
        CUMAT_SAFE_CALL(
            cub::DeviceSegmentedReduce::Reduce(static_cast<void*>(temp_storage.pointer()),
                                               temp_storage_bytes,
                                               iterIn,
                                               iterOut,
                                               numBatches,
...
struct ReductionEvaluator<_Input, _Output, _Axis, _Op, _Scalar, ReductionAlg::Device<MiniBatch>>
{
    static void eval(const MatrixBase<_Input>& in, _Output& out, const _Op& op, const _Scalar& initial)
    {
        ...
        cub::DeviceReduce::Reduce(
            nullptr, temp_storage_bytes, iterIn, iterOut, int(numEntries), op, initial, substreams[0]);
        ...
        for(Index b = 0; b < numBatches; ++b)
        {
            ...
            cub::DeviceReduce::Reduce(temp_storage[i].pointer(),
                                      temp_storage_bytes,
                                      iterIn + (numEntries * b),
                                      iterOut + b,
                                      int(numEntries),
...
struct ReductionEvaluator<_Input, _Output, _Axis, _Op, _Scalar, ReductionAlg::Device<1>>
{
    static void eval(const MatrixBase<_Input>& in, _Output& out, const _Op& op, const _Scalar& initial)
    {
        ...
        cub::DeviceReduce::Reduce(
            nullptr, temp_storage_bytes, iterIn, iterOut, int(numEntries), op, initial, mainStream);
        //perform reduction
        temp_storage = DevicePointer<uint8_t>(temp_storage_bytes);
        for(Index b = 0; b < numBatches; ++b)
        {
            cub::DeviceReduce::Reduce(temp_storage.pointer(),
                                      temp_storage_bytes,
                                      iterIn + (numEntries * b),
                                      iterOut + b,
                                      int(numEntries),
...
namespace kernels
{
    ...
    __global__ void ReduceWarpKernel(
        dim3 virtual_size, _Input input, _Output output, _Op op, _Scalar initial, Index N)
    {
    ...
    static void eval(const MatrixBase<_Input>& in, _Output& out, const _Op& op, const _Scalar& initial)
    {
        ...
        switch(alg)
        {
            case ReductionAlgorithm::Segmented:
                ReductionEvaluator<_Input, _Output, _Axis, _Op, _Scalar, ReductionAlg::Segmented>::eval(
                    in, out, op, initial);
                break;
            case ReductionAlgorithm::Thread:
                ReductionEvaluator<_Input, _Output, _Axis, _Op, _Scalar, ReductionAlg::Thread>::eval(
                    in, out, op, initial);
                break;
            case ReductionAlgorithm::Warp:
                ReductionEvaluator<_Input, _Output, _Axis, _Op, _Scalar, ReductionAlg::Warp>::eval(
                    in, out, op, initial);
                break;
            case ReductionAlgorithm::Block256:
                ReductionEvaluator<_Input, _Output, _Axis, _Op, _Scalar, ReductionAlg::Block<256>>::eval(
                    in, out, op, initial);
                break;
            case ReductionAlgorithm::Device1:
                ReductionEvaluator<_Input, _Output, _Axis, _Op, _Scalar, ReductionAlg::Device<1>>::eval(
                    in, out, op, initial);
                break;
            case ReductionAlgorithm::Device2:
                ReductionEvaluator<_Input, _Output, _Axis, _Op, _Scalar, ReductionAlg::Device<2>>::eval(
                    in, out, op, initial);
                break;
            case ReductionAlgorithm::Device4:
                ReductionEvaluator<_Input, _Output, _Axis, _Op, _Scalar, ReductionAlg::Device<4>>::eval(
                    in, out, op, initial);
                break;
            default:
...
struct ReductionEvaluatorSpecial<_Input, _Output, 0, _Op, _Scalar>
{
    static void eval(const MatrixBase<_Input>& in, _Output& out, const _Op& op, const _Scalar& initial)
    {
        internal::Assignment<_Output,
                             _Input,
                             AssignmentMode::ASSIGN,
                             typename internal::traits<_Output>::DstTag,
...
struct ReductionEvaluatorSpecial<_Input, _Output, Axis::Row | Axis::Column | Axis::Batch, _Op, _Scalar>
{
    ...
    static void eval(const MatrixBase<_Input>& in, _Output& out, const _Op& op, const _Scalar& initial)
    {
        ...
        StridedMatrixInputIterator<_Input> iterIn(
            in,
            isRowMajor ?
                thrust::make_tuple(in.cols(), Index(1), in.cols() * in.rows()) :
                thrust::make_tuple(Index(1), in.rows(), in.rows() * in.cols()));
        ...
        CUMAT_SAFE_CALL(cub::DeviceReduce::Reduce(
            NULL, temp_storage_bytes, iterIn, iterOut, num_items, op, initial, ctx.stream()));
        DevicePointer<uint8_t> temp_storage(temp_storage_bytes);
        CUMAT_SAFE_CALL(
            cub::DeviceReduce::Reduce(static_cast<void*>(temp_storage.pointer()),
                                      temp_storage_bytes,
                                      iterIn,
                                      iterOut,
                                      num_items,
...
#define SPECIALIZE_ALG(alg)                                                                                 \
    ...
    {                                                                                                       \
        static void eval(const MatrixBase<_Input>& in, _Output& out, const _Op& op, const _Scalar& initial) \
        {                                                                                                   \
            ReductionEvaluatorSpecial<_Input, _Output, 0, _Op, _Scalar>::eval(                              \
                in, out, op, initial);                                                                      \
        }                                                                                                   \
    };                                                                                                      \
                                                                                                            \
    ...
    {                                                                                                       \
        static void eval(const MatrixBase<_Input>& in, _Output& out, const _Op& op, const _Scalar& initial) \
        {                                                                                                   \
            ReductionEvaluatorSpecial<_Input, _Output, Axis::All, _Op, _Scalar>::eval(                      \
                in, out, op, initial);                                                                      \
        }                                                                                                   \
    }
...
template <typename _Child, typename _ReductionOp, typename _Algorithm>
class ReductionOp_DynamicSwitched
    : public MatrixBase<ReductionOp_DynamicSwitched<_Child, _ReductionOp, _Algorithm>>
{
  public:
  ...
  public:
    ReductionOp_DynamicSwitched(const MatrixBase<_Child>& child,
                                int                       axis,
                                const _ReductionOp&       op,
    ...
    void evalTo(MatrixBase<Derived>& m) const
    {
        ...
        static_assert(Mode == AssignmentMode::ASSIGN,
                      "Currently, only AssignmentMode::ASSIGN is supported");
        ...
        CUMAT_LOG_DEBUG("Evaluate reduction expression "
                        << typeid(derived()).name() << "\n rows=" << m.rows()
                        << ", cols=" << m.cols() << ", batches=" << m.batches()
                        << ", axis=" << ((axis_ & Axis::Row) ? "R" : "")
                        << ((axis_ & Axis::Column) ? "C" : "")
                        << ((axis_ & Axis::Batch) ? "B" : ""));
        ...
        {
            case 0:
                internal::ReductionEvaluator<_Child, Derived, 0, _ReductionOp, Scalar, _Algorithm>::eval(
                    child_, m.derived(), op_, initialValue_);
                break;
            case 1:
                internal::ReductionEvaluator<_Child, Derived, 1, _ReductionOp, Scalar, _Algorithm>::eval(
                    child_, m.derived(), op_, initialValue_);
                break;
            case 2:
                internal::ReductionEvaluator<_Child, Derived, 2, _ReductionOp, Scalar, _Algorithm>::eval(
                    child_, m.derived(), op_, initialValue_);
                break;
            case 3:
                internal::ReductionEvaluator<_Child, Derived, 3, _ReductionOp, Scalar, _Algorithm>::eval(
                    child_, m.derived(), op_, initialValue_);
                break;
            case 4:
                internal::ReductionEvaluator<_Child, Derived, 4, _ReductionOp, Scalar, _Algorithm>::eval(
                    child_, m.derived(), op_, initialValue_);
                break;
            case 5:
                internal::ReductionEvaluator<_Child, Derived, 5, _ReductionOp, Scalar, _Algorithm>::eval(
                    child_, m.derived(), op_, initialValue_);
                break;
            case 6:
                internal::ReductionEvaluator<_Child, Derived, 6, _ReductionOp, Scalar, _Algorithm>::eval(
                    child_, m.derived(), op_, initialValue_);
                break;
            case 7:
                internal::ReductionEvaluator<_Child, Derived, 7, _ReductionOp, Scalar, _Algorithm>::eval(
                    child_, m.derived(), op_, initialValue_);
                break;
            default:
                throw std::invalid_argument(__FILE__ ":" CUMAT_STR(__LINE__) ": Invalid argument, axis must be between 0 and 7, but is "
                                            + std::to_string(axis_));
        }
...
struct traits<ReductionOp_StaticSwitched<_Child, _ReductionOp, _Axis, _Algorithm>>
{
    ...
    enum
    {
        ...
        BatchesAtCompileTime =
            ((_Axis & Axis::Batch) ? 1 : internal::traits<_Child>::BatchesAtCompileTime),
        AccessFlags = 0
    };
    typedef ReductionSrcTag SrcTag;
    typedef DeletedDstTag   DstTag;
    ...
    : public MatrixBase<ReductionOp_StaticSwitched<_Child, _ReductionOp, _Axis, _Algorithm>>
{
  ...
  public:
    ReductionOp_StaticSwitched(const MatrixBase<_Child>& child,
                               const _ReductionOp&       op,
                               const Scalar&             initialValue)
        : child_(child.derived())
        , op_(op)
    ...
    void evalTo(MatrixBase<Derived>& m) const
    {
        ...
        CUMAT_LOG_DEBUG("Evaluate reduction expression "
                        ...
                        << ", batches=" << m.batches() << "("
                        << internal::traits<_Child>::BatchesAtCompileTime << ")"
                        << ", axis=" << ((AxisSimplified & Axis::Row) ? "R" : "")
                        << ((AxisSimplified & Axis::Column) ? "C" : "")
                        << ((AxisSimplified & Axis::Batch) ? "B" : ""));
