...
class CholeskyDecomposition
    : public DecompositionBase<CholeskyDecomposition<_MatrixType>>
{
  public:
    using Scalar = typename internal::traits<_MatrixType>::Scalar;
  ...
  public:
    ...
    */
    void compute(const MatrixBase<_MatrixType>& matrix, bool inplace = false)
    {
        ...
        for(Index batch = 0; batch < batches; ++batch)
        {
            internal::CusolverApi::current().cusolverPotrf(
                ...
                n,
                internal::CusolverApi::cast(decompositedMatrix_.data() + batch * n * n),
                lda,
                devInfo.data() + batch);
        }
        ...
        for(Index i = 0; i < batches; ++i)
        {
            if(singular_[i] < 0)
            {
                throw cuda_error(internal::ErrorHelpers::format(
                    "Potrf failed at batch %d, parameter %d was invalid", i, -singular_[i]));
            }
            else if(singular_[i] > 0)
            {
                throw cuda_error(internal::ErrorHelpers::format("Potrf failed at batch %d, the leaading minor %d was was not positive definite",
                                                                i,
                                                                singular_[i]));
            }
        }
    ...
    DeterminantMatrix determinant() const
    {
        ...
        return decompositedMatrix_.diagonal()
            ...
            .cwiseAbs2();  //multiply diagonal elements
    ...
    */
    DeterminantMatrix logDeterminant() const
    {
        ...
        return (decompositedMatrix_.diagonal().cwiseLog().template sum<Axis::Row | Axis::Column>())
    ...
    void _solve_impl(const MatrixBase<_RHS>& rhs, MatrixBase<_Target>& target) const
    {
        ...
        CUMAT_SAFE_CALL(cudaMemcpyAsync(&hostInfo[0],
                                        devInfo.pointer(),
                                        sizeof(int) * batches,
        ...
        for(Index i = 0; i < batches; ++i)
        {
            ...
            {
                throw cuda_error(internal::ErrorHelpers::format(
                    "Potrs failed at batch %d, parameter %d was invalid", i, -hostInfo[i]));
            }
        }
    }
};
