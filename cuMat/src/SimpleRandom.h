...
namespace kernels
{
    ...
    struct RandNext<long long>
    {
        ...
        {
            long long v = ((long long)(RandNextHelper::eval(32, seed)) << 32)
                          + RandNextHelper::eval(32, seed);
    ...
    struct RandNext<double>
    {
        ...
        {
            double v = (((long)(RandNextHelper::eval(26, seed)) << 27)
                        + RandNextHelper::eval(27, seed))
                       / (double)(1LL << 53);
            return (v * (max - min)) + min;
        }
    };
...
class SimpleRandom
{
  ...
  public:
        ...
        : state_(numStates)
    {
        ...
        CUMAT_SAFE_CALL(cudaMemcpyAsync(state_.pointer(),
                                        v.data(),
                                        numStates * sizeof(internal::kernels::state_t),
     ...
     */
    template <typename _Derived, typename _Scalar = typename internal::traits<_Derived>::Scalar>
    void fillUniform(MatrixBase<_Derived>& m,
                     const _Scalar& min = internal::MinMaxDefaults<_Scalar>::min(),
                     const _Scalar& max = internal::MinMaxDefaults<_Scalar>::max())
    {
#if CUMAT_NVCC == 1
        if(m.size() == 0)
            return;
        ...
        KernelLaunchConfig              cfg = ctx.createLaunchConfig1D(
            m.size(), internal::kernels::RandomEvaluationKernel<ActualType, _Scalar>);
        internal::kernels::RandomEvaluationKernel<ActualType, _Scalar>
            <<<1, cfg.thread_per_block.x, 0, ctx.stream()>>>(
                cfg.virtual_size, m.derived(), min, max, state_.pointer());
        CUMAT_CHECK_ERROR();
#else
