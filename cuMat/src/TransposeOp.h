...
template <typename Scalar, bool _IsConjugated>
__device__ CUMAT_STRONG_INLINE Scalar conjugateCoeff(const Scalar& val)
{
    return val;
};
template <>
__device__ CUMAT_STRONG_INLINE cfloat conjugateCoeff<cfloat, true>(const cfloat& val)
{
    return conj(val);
}
template <>
__device__ CUMAT_STRONG_INLINE cdouble conjugateCoeff<cdouble, true>(const cdouble& val)
{
    return conj(val);
}
...
void directTranspose(Scalar*           dst,
                     const Scalar*     src,
                     Index             rows,
                     Index             cols,
                     Index             batches,
                     cublasOperation_t transOp = CUBLAS_OP_T)
{
    ...
    for(Index batch = 0; batch < batches; ++batch)
    {
        internal::CublasApi::current().cublasGeam(
            ...
            internal::CublasApi::cast(A + batch * batch_offset),
            lda,
            internal::CublasApi::cast(&beta),
            internal::CublasApi::cast(B),
            ldb,
            internal::CublasApi::cast(C + batch * batch_offset),
            ldc);
    }
...
class TransposeOp : public CwiseOp<TransposeOp<_Derived, _Conjugated>>
{
  public:
    ...
    enum
    {
        ...
        IsConjugated = _Conjugated
                       && internal::NumTraits<typename internal::traits<_Derived>::Scalar>::IsComplex
    };
  ...
  public:
    ...
    const _Derived& getUnderlyingMatrix() const { return matrix_; }
...
struct Assignment<_Dst, _Src, _AssignmentMode, _DstTag, TransposeSrcTag>
{
    ...
    static CUMAT_STRONG_INLINE void evalToImpl(const Op&            src,
                                               MatrixBase<Derived>& m,
                                               std::false_type,
                                               std::integral_constant<bool, _Conj>)
    {
        CUMAT_ERROR_IF_NO_NVCC(cwise_transpose)
        //don't pass _Conj further, it is equal to IsConjugated
        Assignment<Derived, Op, AssignmentMode::ASSIGN, typename Derived::DstTag, CwiseSrcTag>::assign(
            m.derived(), src);
    }
    ...
    static CUMAT_STRONG_INLINE void evalToImpl(const Op& src,
                                               Matrix<Scalar, _Rows, _Columns, _Batches, Op::Flags>& m,
                                               std::true_type,
                                               std::false_type)
    {
        m = Matrix<Scalar, _Rows, _Columns, _Batches, Op::Flags>(
            src.getUnderlyingMatrix().dataPointer(), src.rows(), src.cols(), src.batches());
    }
    ...
    template <int _Rows, int _Columns, int _Batches>
    static void evalToImplDirect(const Op& src,
                                 Matrix<Scalar, _Rows, _Columns, _Batches, Op::OriginalFlags>& mat,
                                 std::true_type)
    {
        ...
        int m = static_cast<int>(Op::OriginalFlags == ColumnMajor ? mat.rows() :
                                                                    mat.cols());
        int n = static_cast<int>(Op::OriginalFlags == ColumnMajor ? mat.cols() :
                                                                    mat.rows());

        //perform transposition
        internal::directTranspose(
            mat.data(), src.getUnderlyingMatrix().data(), m, n, src.batches(), transA);
    }
    template <int _Rows, int _Columns, int _Batches>
    static CUMAT_STRONG_INLINE void evalToImplDirect(
        const Op&                                                     src,
        Matrix<Scalar, _Rows, _Columns, _Batches, Op::OriginalFlags>& mat,
        std::false_type)
    {
    ...
    static CUMAT_STRONG_INLINE void evalToImpl(
        const Op&                                                     src,
        Matrix<Scalar, _Rows, _Columns, _Batches, Op::OriginalFlags>& mat,
        std::true_type,
        std::integral_constant<bool, _Conj>)
    {
        ...
        evalToImplDirect(
            src, mat, std::integral_constant<bool, internal::NumTraits<Scalar>::IsCudaNumeric>());
    }
    ...
    static void assign(_Dst& dst, const _Src& src)
    {
        ...
        evalToImpl(src.derived(),
                   dst.derived(),
                   std::integral_constant<bool, SrcActual::IsMatrix>(),
                   std::integral_constant<bool, SrcActual::IsConjugated>());
    }
};
}  // namespace internal
