...
#define CUMAT_ERROR_IF_NO_NVCC(name)                                           \
    {                                                                          \
        THIS_FUNCTION_REQUIRES_THE_FILE_TO_BE_COMPILED_WITH_NVCC name;         \
    }
#else
#define CUMAT_ERROR_IF_NO_NVCC(name)
#endif
...
#define CUMAT_ASSERT_ARGUMENT(x)                                               \
    if(!(x))                                                                   \
        throw std::invalid_argument(                                           \
            __FILE__ ":" CUMAT_STR(__LINE__) ": Invalid argument: " #x);
#define CUMAT_ASSERT_BOUNDS(x)                                                 \
    if(!(x))                                                                   \
        throw std::out_of_range(__FILE__ ":" CUMAT_STR(__LINE__) "Out of bounds: " #x);
#define CUMAT_ASSERT_ERROR(x)                                                  \
    if(!(x))                                                                   \
        throw std::runtime_error(__FILE__ ":" CUMAT_STR(__LINE__) "Runtime Error: " #x);
#define CUMAT_ASSERT_DIMENSION(x)                                              \
    if(!(x))                                                                   \
        throw std::invalid_argument(                                           \
            __FILE__ ":" CUMAT_STR(__LINE__) ": Invalid dimensions: " #x);
