...
class IterativeSolverBase : public SolverBase<_SolverImpl>
{
  ...
  public:
    ...
    IterativeSolverBase(const MatrixBase<MatrixType>& matrix)
        ...
        , preconditioner_(matrix.derived())
    {
        init();
        initialized_ = true;
    }
    ...
    */
    ...
    CUMAT_STRONG_INLINE SolveWithGuessOp<_SolverImpl, Rhs, Guess> solveWithGuess(
...
class SolveWithGuessOp : public MatrixBase<SolveWithGuessOp<_Solver, _RHS, _Guess>>
{
  ...
  public:
    SolveWithGuessOp(const _Solver&            decomposition,
                     const MatrixBase<_RHS>&   rhs,
                     const MatrixBase<_Guess>& guess)
        : decomposition_(decomposition)
        , rhs_(rhs.derived())
        , guess_(guess.derived())
    {
        CUMAT_STATIC_ASSERT(
            (std::is_same<typename internal::NumTraits<typename _Solver::Scalar>::ElementalType,
        ...
        CUMAT_STATIC_ASSERT(CUMAT_IMPLIES(_Solver::Batches > 1 && _RHS::Batches > 0,
                                          _Solver::Batches == _RHS::Batches),
                            "Static count of batches must match");  //note: _Solver::Batches>1 to allow broadcasting
        CUMAT_STATIC_ASSERT(CUMAT_IMPLIES(_Solver::Rows > 0 && _Solver::Columns > 0,
                                          _Solver::Rows == _RHS::Columns),
                            "Static count of rows and columns must be equal (square matrix)");
        CUMAT_STATIC_ASSERT(CUMAT_IMPLIES(_Solver::Rows > 0 && _RHS::Rows > 0,
                                          _Solver::Rows == _RHS::Rows),
                            "Left and right hand side are not compatible");

        CUMAT_ASSERT(CUMAT_IMPLIES(_Solver::Batches != 1,
                                   decomposition.batches() == rhs.batches())
                     && "Number of batches must be compatible");
        CUMAT_ASSERT(decomposition.rows() == decomposition.cols()
                     && "Matrix must be square");  //TODO: relax that for Least Squares problems?
        CUMAT_ASSERT(decomposition.rows() == rhs.rows()
                     && "Matrix must be compatible with the right hand side");
    }
...
struct Assignment<_Dst, _Src, _Mode, DenseDstTag, SolveWithGuessSrcTag>
{
    ...
    {
        static_assert(_Mode == AssignmentMode::ASSIGN,
                      "Decompositions only support AssignmentMode::ASSIGN (operator=)");
        src.derived().getDecomposition()._solve_with_guess_impl(
            src.derived().getRhs().derived(),
            dst.derived().derived(),
            src.derived().getGuess().derived());
    }
};
}  // namespace internal
