    ...
    static const char* getErrorName(cusolverStatus_t status)
    {
        switch(status)
        {
            case CUSOLVER_STATUS_NOT_INITIALIZED:
                return "CUSOLVER_STATUS_NOT_INITIALIZED: cuSOLVER was not initialized";
            case CUSOLVER_STATUS_ALLOC_FAILED:
                return "CUSOLVER_STATUS_ALLOC_FAILED: resource allocation failed";
            case CUSOLVER_STATUS_INVALID_VALUE:
                return "CUSOLVER_STATUS_INVALID_VALUE: invalid value was passed as argument";
            case CUSOLVER_STATUS_ARCH_MISMATCH:
                return "CUSOLVER_STATUS_ARCH_MISMATCH: device architecture not supported";
            case CUSOLVER_STATUS_EXECUTION_FAILED:
                return "CUSOLVER_STATUS_EXECUTION_FAILED: general kernel launch failure";
            case CUSOLVER_STATUS_INTERNAL_ERROR:
                return "CUSOLVER_STATUS_INTERNAL_ERROR: an internal error occured";
            case CUSOLVER_STATUS_MATRIX_TYPE_NOT_SUPPORTED:
                return "CUSOLVER_STATUS_MATRIX_TYPE_NOT_SUPPORTED: matrix type is not supported";
            default:
                return "";
        }
    }
    static void cusolverSafeCall(cusolverStatus_t status, const char* file, const int line)
    {
        if(CUSOLVER_STATUS_SUCCESS != status)
        {
            std::string msg = ErrorHelpers::format(
                "cusolverSafeCall() failed at %s:%i : %s\n", file, line, getErrorName(status));
...
#define CUSOLVER_MAKE_WRAPPER(name, factory)                                                  \
    void name##Impl factory(float, cusolverDnS##name) void name##Impl factory(                \
        double, cusolverDnD##name) void name##Impl factory(cuComplex, cusolverDnC##name) void \
        name##Impl factory(cuDoubleComplex, cusolverDnZ##name)

    //POTRF

  private:
#define CUSOLVER_POTRF_FACTORY(scalar, op)                                           \
    (cublasFillMode_t uplo, int n, scalar* A, int lda, int* devInfo)                 \
    {                                                                                \
        int Lwork;                                                                   \
        CUSOLVER_SAFE_CALL(op##_bufferSize(handle_, uplo, n, A, lda, &Lwork));       \
        scalar* workspace =                                                          \
            static_cast<scalar*>(ctx_->mallocDevice(sizeof(scalar) * Lwork));        \
...
#undef CUSOLVER_POTRF_FACTORY
  public:
    /**
         ...
         */
    template <typename _Scalar>
    void cusolverPotrf(cublasFillMode_t uplo, int n, _Scalar* A, int lda, int* devInfo)
    {
...
#define CUSOLVER_POTRS_FACTORY(scalar, op)                                                               \
    (cublasFillMode_t uplo, int n, int nrhs, const scalar* A, int lda, scalar* B, int ldb, int* devInfo) \
    {                                                                                                    \
...
#undef CUSOLVER_POTRS_FACTORY
  public:
    /**
        ...
        */
    template <typename _Scalar>
    void cusolverPotrs(cublasFillMode_t uplo,
                       int              n,
                       int              nrhs,
                       const _Scalar*   A,
                       int              lda,
...
#define CUSOLVER_GETRF_FACTORY(scalar, op)                                          \
    (int m, int n, scalar* A, int lda, int* devIpiv, int* devInfo)                  \
    {                                                                               \
        int Lwork;                                                                  \
...
#undef CUSOLVER_GETRF_FACTORY
  public:
    /**
        ...
        */
    template <typename _Scalar>
    void cusolverGetrf(int m, int n, _Scalar* A, int lda, int* devIpiv, int* devInfo)
    {
...
#undef CUSOLVER_GETRS_FACTORY
  public:
    /**
        ...
        */
    template <typename _Scalar>
    void cusolverGetrs(cublasOperation_t trans,
                       int               n,
                       int               nrhs,
                       const _Scalar*    A,
                       int               lda,
