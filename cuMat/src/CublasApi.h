...
class CublasApi
{
  ...
  private:
    ...
    static const char* getErrorName(cublasStatus_t status)
    {
        switch(status)
        {
            case CUBLAS_STATUS_NOT_INITIALIZED:
                return "CUBLAS_STATUS_NOT_INITIALIZED: cuBLAS was not initialized";
            case CUBLAS_STATUS_ALLOC_FAILED:
                return "CUBLAS_STATUS_ALLOC_FAILED: resource allocation failed";
            case CUBLAS_STATUS_INVALID_VALUE:
                return "CUBLAS_STATUS_INVALID_VALUE: invalid value was passed as argument";
            case CUBLAS_STATUS_ARCH_MISMATCH:
                return "CUBLAS_STATUS_ARCH_MISMATCH: device architecture not supported";
            case CUBLAS_STATUS_MAPPING_ERROR:
                return "CUBLAS_STATUS_MAPPING_ERROR: access to GPU memory failed";
            case CUBLAS_STATUS_EXECUTION_FAILED:
                return "CUBLAS_STATUS_EXECUTION_FAILED: general kernel launch failure";
            case CUBLAS_STATUS_INTERNAL_ERROR:
                return "CUBLAS_STATUS_INTERNAL_ERROR: an internal error occured";
            case CUBLAS_STATUS_NOT_SUPPORTED:
                return "CUBLAS_STATUS_NOT_SUPPORTED: functionality is not supported";
            case CUBLAS_STATUS_LICENSE_ERROR:
                return "CUBLAS_STATUS_LICENSE_ERROR: required licence was not found";
            default:
                return "";
        }
    }
    static void cublasSafeCall(cublasStatus_t status, const char* file, const int line)
    {
        ...
        {
            std::string msg = ErrorHelpers::format(
                "cublasSafeCall() failed at %s:%i : %s\n", file, line, getErrorName(status));
...
#define CUBLAS_SAFE_CALL(err) cublasSafeCall(err, __FILE__, __LINE__)
  ...
  public:
    ...
    static _Scalar* cast(_Scalar* p)
    {
        return p;
    }
    static cuFloatComplex* cast(cfloat* p)
    {
        return reinterpret_cast<cuFloatComplex*>(p);
    }
    static const cuFloatComplex* cast(const cfloat* p)
    {
        return reinterpret_cast<const cuFloatComplex*>(p);
    }
    static cuDoubleComplex* cast(cdouble* p)
    {
        return reinterpret_cast<cuDoubleComplex*>(p);
    }
    static const cuDoubleComplex* cast(const cdouble* p)
    {
        return reinterpret_cast<const cuDoubleComplex*>(p);
    }
...
#define CUBLAS_MAKE_WRAPPER(name, factory)                                                    \
    void cublas##name##Impl factory(float, cublasS##name) void cublas##name##Impl factory(    \
        double, cublasD##name) void cublas##name##Impl factory(cuComplex, cublasC##name) void \
        cublas##name##Impl factory(cuDoubleComplex, cublasZ##name)

#define CUBLAS_MAKE_WRAPPER_COMPLEX(name, factory)                                             \
    void cublas##name##Impl factory(cuComplex, cublasC##name) void cublas##name##Impl factory( \
        cuDoubleComplex, cublasZ##name)
...
#define CUBLAS_GEAM_FACTORY(scalar, op)                                              \
    ...
    {                                                                                \
        CUBLAS_SAFE_CALL(                                                            \
            op(handle_, transA, transB, m, n, alpha, A, lda, beta, B, ldb, C, ldc)); \
    }
...
#undef CUBLAS_GEAM_FACTORY
  public:
    /**
         ...
         */
    template <typename _Scalar>
    void cublasGeam(cublasOperation_t transA,
                    cublasOperation_t transB,
                    int               m,
                    int               n,
                    const _Scalar*    alpha,
...
#define CUBLAS_GEMM_FACTORY(scalar, op)                                                 \
    ...
    {                                                                                   \
        CUBLAS_SAFE_CALL(                                                               \
            op(handle_, transA, transB, m, n, k, alpha, A, lda, B, ldb, beta, C, ldc)); \
    }
...
#undef CUBLAS_GEMM_FACTORY
  public:
    /**
        ...
        */
    template <typename _Scalar>
    void cublasGemm(cublasOperation_t transA,
                    cublasOperation_t transB,
                    int               m,
                    int               n,
                    int               k,
...
#undef CUBLAS_GEMM3M_FACTORY
  public:
    /**
        ...
        */
    template <typename _Scalar>
    void cublasGemm3m(cublasOperation_t transA,
                      cublasOperation_t transB,
                      int               m,
                      int               n,
                      int               k,
...
#define CUBLAS_BATCHED_GEMM_FACTORY(scalar, op)                                                                             \
    ...
    {                                                                                                                       \
        CUBLAS_SAFE_CALL(op(                                                                                                \
            handle_, transA, transB, m, n, k, alpha, A, lda, strideA, B, ldb, strideB, beta, C, ldc, strideC, batchCount)); \
    }
...
#undef CUBLAS_BATCHED_GEMM_FACTORY
  public:
    /**
        ...
        */
    template <typename _Scalar>
    void cublasGemmBatched(cublasOperation_t transA,
                           cublasOperation_t transB,
                           int               m,
                           int               n,
                           int               k,
